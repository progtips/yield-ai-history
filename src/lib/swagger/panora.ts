export const panoraOpenApi = {
  ...
  servers: [{ url: '/api/panora' }],
  paths: {
    '/tokenList': {
      get: {
        summary: 'Get Token List',
        description:
          'Returns the list of Panora tokens for the specified network.',
        parameters: [
          {
            name: 'chainId',
            in: 'query',
    ...
    schemas: {
      Token: {
        ...
        properties: {
          ...
          logoUrl: {
            type: 'string',
            example: 'https://assets.panora.exchange/tokens/aptos/APT.svg',
          },
          websiteUrl: {
            type: ['string', 'null'],
            example: 'https://aptosfoundation.org',
          },
          panoraUI: { type: 'boolean', example: true },
          panoraTags: {
            type: 'array',
            items: { type: 'string' },
            example: ['Native', 'Verified'],
          },
