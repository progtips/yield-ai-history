...
export function calculateProtocolProfit(
  ...
  const sortedTransactions = protocolTransactions.sort((a, b) => {
    const timeA =
      typeof a.timestamp === 'string' ? parseFloat(a.timestamp) : a.timestamp;
    const timeB =
      typeof b.timestamp === 'string' ? parseFloat(b.timestamp) : b.timestamp;
    return timeA - timeB;
  });
  ...
  sortedTransactions.forEach(tx => {
    ...
    const { amount, token } = extractTransactionAmount(
      tx._rawData || tx,
      tx.from
    );
  ...
  const profitPercentage =
    totalDeposits > 0 ? (profit / totalDeposits) * 100 : 0;

  // Рассчитываем среднее значение транзакции
  const averageTransactionValue =
    protocolTransactions.length > 0
      ? (totalDeposits +
          totalWithdrawals +
          totalClaims +
          totalSwaps +
  ...
  const activeDays = Math.ceil(
    (new Date(lastTransactionDate).getTime() -
 ...
 */
function getProtocolNameByFunction(
  functionPath: string,
  recipientAddress: string
): string {
  console.log('[DEBUG] getProtocolNameByFunction вызвана с:', {
    functionPath,
    recipientAddress,
  });
  ...
  if (!functionPath || functionPath === 'Unknown') {
    console.log(
      '[DEBUG] getProtocolNameByFunction возвращает recipientAddress:',
  ...
  if (parts.length < 2) {
    console.log(
      '[DEBUG] getProtocolNameByFunction возвращает recipientAddress или functionPath:',
      recipientAddress || functionPath
    );
  ...
  if (protocol) {
    console.log(
      '[DEBUG] getProtocolNameByFunction возвращает протокол:',
      protocol
    );
    return protocol;
  }
  ...
  if (
    ...
    console.log(
      '[DEBUG] getProtocolNameByFunction возвращает recipientAddress (неизвестный протокол):',
      recipientAddress
    );
...
export function extractTransactionAmount(
...
): { amount: number; token: string; decimals: number } {
  const protocol = getProtocolNameByFunction(
    tx.payload?.function || '',
    tx.to || ''
  );

  console.log(
    '[DEBUG] extractTransactionAmount - функция:',
    tx.payload?.function
  );
  console.log('[DEBUG] extractTransactionAmount - протокол:', protocol);
  console.log(
    '[DEBUG] extractTransactionAmount - payload type:',
    tx.payload?.type
  );
  ...
  if (
    tx.hash ===
    '0xf96adf5f9270a8e6d1f0bcca38d6678ed4153e0289474d1832ff4394f1cb043f'
  ) {
    console.log(
      '[DEBUG] === СПЕЦИАЛЬНАЯ ОБРАБОТКА В extractTransactionAmount ==='
    );
  ...
  if (!tx.payload?.function && tx.payload?.type === 'entry_function_payload') {
    console.log(
      '[DEBUG] Функция не определена, но это entry_function_payload, используем extractGenericAmount'
    );
...
function extractEchelonAmount(
  ...
  if (debugCallback)
    debugCallback(`[DEBUG] Events count: ${tx.events?.length || 0}`);
  ...
  if (!tx.events || !Array.isArray(tx.events)) {
    if (debugCallback)
      debugCallback(
        '[DEBUG] extractEchelonAmount - нет событий, возвращаем значения по умолчанию'
      );
  ...
  if (
    ...
    if (debugCallback)
      debugCallback(
        '[DEBUG] === СПЕЦИАЛЬНАЯ ОБРАБОТКА В extractEchelonAmount ==='
      );
    if (debugCallback) debugCallback(`[DEBUG] Hash: ${tx.hash}`);
    if (debugCallback)
      debugCallback(`[DEBUG] Function: ${tx.payload?.function}`);
    if (debugCallback)
      debugCallback(
        `[DEBUG] Все события: ${JSON.stringify(tx.events, null, 2)}`
      );
    if (debugCallback)
      debugCallback(
        `[DEBUG] Все изменения: ${JSON.stringify(tx.changes, null, 2)}`
      );
    if (debugCallback)
      debugCallback(`[DEBUG] Payload: ${JSON.stringify(tx.payload, null, 2)}`);
  }

  // Шаг 1: Ищем события Deposit и Withdraw
  const depositEvents = tx.events.filter(
    (event: any) =>
      event.type === '0x1::fungible_asset::Deposit' ||
      event.type.includes('fungible_asset::Deposit')
  );

  const withdrawEvents = tx.events.filter(
    (event: any) =>
  ...
  if (tx.changes) {
    if (debugCallback)
      debugCallback('[DEBUG] Анализируем changes для определения токена...');
    if (debugCallback)
      debugCallback(
        `[DEBUG] Все changes: ${JSON.stringify(tx.changes, null, 2)}`
      );
    ...
    if (marketChange?.data?.data?.asset_name) {
      ...
      if (debugCallback)
        debugCallback(
          `[DEBUG] Найден asset_name в Market change: ${assetName}`
        );
      ...
      if (tokenInfo) {
        ...
        if (debugCallback)
          debugCallback(`[DEBUG] Установлен токен из Market change: ${token}`);
      }
    }

    // Ищем токен в CoinStore изменениях для всех транзакций Echelon
    const coinStoreChanges = tx.changes.filter(
      (change: any) =>
    ...
    if (debugCallback)
      debugCallback(
        `[DEBUG] Найдено CoinStore изменений: ${coinStoreChanges.length}`
      );
    ...
    for (const coinStoreChange of coinStoreChanges) {
      if (coinStoreChange?.data?.data?.coin?.type) {
        ...
        if (debugCallback)
          debugCallback(`[DEBUG] Найден тип токена в CoinStore: ${coinType}`);
    ...
    const fungibleStoreChanges = tx.changes.filter(
      (change: any) =>
    ...
    if (debugCallback)
      debugCallback(
        `[DEBUG] Найдено FungibleStore изменений: ${fungibleStoreChanges.length}`
      );
    ...
    for (const fungibleStoreChange of fungibleStoreChanges) {
      if (fungibleStoreChange?.data?.data?.metadata?.inner) {
        ...
        if (debugCallback)
          debugCallback(
            `[DEBUG] Найден metadata.inner в FungibleStore: ${metadataInner}`
          );
        ...
        if (
          ...
          decimals = getTokenDecimals('thAPT', debugCallback);
          if (debugCallback)
            debugCallback('[DEBUG] Установлен thAPT из FungibleStore metadata');
          break;
        } else if (metadataInner === '0x1::aptos_coin::AptosCoin') {
          token = 'APT';
          decimals = getTokenDecimals('APT', debugCallback);
          if (debugCallback)
            debugCallback('[DEBUG] Установлен APT из FungibleStore metadata');
          break;
        }
      }
    }
  ...
  if (token === 'APT') {
    if (debugCallback)
      debugCallback('[DEBUG] Ищем PriceEvent для определения токена...');
    const priceEvent = tx.events.find(
      (event: any) =>
    ...
    if (priceEvent && priceEvent.data && priceEvent.data.coin_name) {
      if (debugCallback)
        debugCallback(
          `[DEBUG] Найден PriceEvent с coin_name: ${priceEvent.data.coin_name}`
        );
      const tokenInfo = getTokenInfoByCoinName(
        priceEvent.data.coin_name,
        debugCallback
      );
      if (tokenInfo) {
        token = tokenInfo.symbol;
        decimals = tokenInfo.decimals;
        if (debugCallback)
          debugCallback(`[DEBUG] Установлен токен из PriceEvent: ${token}`);
      }
    }
  }
  ...
  if (token === 'APT' && tx.events) {
    if (debugCallback)
      debugCallback(
        '[DEBUG] Анализируем все события для определения токена...'
      );
    if (debugCallback)
      debugCallback(
        `[DEBUG] Все события: ${JSON.stringify(tx.events, null, 2)}`
    ...
    for (const event of tx.events) {
      if (event.type) {
        ...
        if (event.type.includes('staking::ThalaAPT')) {
          ...
          if (debugCallback)
            debugCallback(
              `[DEBUG] Установлен thAPT из типа события: ${event.type}`
            );
          break;
          ...
          if (debugCallback)
            debugCallback(
              `[DEBUG] Установлен sthAPT из типа события: ${event.type}`
            );
          break;
        }
        ...
        if (typeMatch) {
          ...
          if (debugCallback)
            debugCallback(`[DEBUG] Найден generic тип в событии: ${coinType}`);

          if (coinType.includes('staking::ThalaAPT')) {
            token = 'thAPT';
            decimals = 8;
            if (debugCallback)
              debugCallback('[DEBUG] Установлен thAPT из generic типа события');
            break;
          } else if (coinType.includes('staking::StakedThalaAPT')) {
            token = 'sthAPT';
            decimals = 8;
            if (debugCallback)
              debugCallback(
                '[DEBUG] Установлен sthAPT из generic типа события'
              );
            break;
          }
        }
        ...
        if (event.data) {
          if (debugCallback)
            debugCallback(
              `[DEBUG] Анализируем данные события: ${JSON.stringify(event.data, null, 2)}`
            );
          ...
          for (const tokenField of possibleTokenFields) {
            if (tokenField) {
              if (debugCallback)
                debugCallback(
                  `[DEBUG] Найден токен в данных события: ${tokenField}`
                );
              ...
              if (typeof tokenField === 'string') {
                if (tokenField.includes('staking::ThalaAPT')) {
                  ...
                  decimals = getTokenDecimals('thAPT', debugCallback);
                  if (debugCallback)
                    debugCallback('[DEBUG] Установлен thAPT из данных события');
                  break;
                  ...
                  if (debugCallback)
                    debugCallback(
                      '[DEBUG] Установлен sthAPT из данных события'
                    );
                  break;
                }
                ...
                if (tokenInfo) {
                  ...
                  if (debugCallback)
                    debugCallback(
                      `[DEBUG] Установлен токен из данных события: ${token}`
                    );
                  break;
                }
              }
  ...
  if (token === 'APT' && functionName.includes('claim')) {
    ...
    if (depositEvent?.type) {
      ...
      if (typeMatch) {
        const coinType = typeMatch[1];
        if (debugCallback)
          debugCallback(`[DEBUG] Найден тип токена в событии: ${coinType}`);
  ...
  if (token === 'APT' && tx.payload?.type_arguments) {
    if (debugCallback)
      debugCallback(
        `[DEBUG] Анализируем type_arguments: ${JSON.stringify(tx.payload.type_arguments, null, 2)}`
      );

    for (const typeArg of tx.payload.type_arguments) {
      if (debugCallback)
        debugCallback(`[DEBUG] Проверяем type_argument: ${typeArg}`);

      if (typeArg.includes('staking::ThalaAPT')) {
        token = 'thAPT';
        decimals = getTokenDecimals('thAPT', debugCallback);
        if (debugCallback)
          debugCallback('[DEBUG] Установлен thAPT из type_arguments');
        break;
      } else if (typeArg.includes('staking::StakedThalaAPT')) {
        token = 'sthAPT';
        decimals = getTokenDecimals('sthAPT', debugCallback);
        if (debugCallback)
          debugCallback('[DEBUG] Установлен sthAPT из type_arguments');
        break;
      } else if (typeArg.includes('usde::USDe')) {
        token = 'USDe';
        decimals = 6;
        if (debugCallback)
          debugCallback('[DEBUG] Установлен USDe из type_arguments');
        break;
      } else if (typeArg.includes('staked_usde::StakedUSDe')) {
        token = 'sUSDe';
        decimals = 6;
        if (debugCallback)
          debugCallback('[DEBUG] Установлен sUSDe из type_arguments');
        break;
      }
    }
  }
  ...
  if (functionName.includes('supply') || functionName.includes('deposit')) {
    ...
    functionName.includes('scripts::claim_reward')
  ) {
    // Операция получения наград
    if (debugCallback)
      debugCallback(`[DEBUG] Обрабатываем claim операцию: ${functionName}`);
    if (debugCallback)
      debugCallback(
        `[DEBUG] Deposit events: ${JSON.stringify(depositEvents, null, 2)}`
      );
    ...
    if (depositEvents.length > 0) {
      ...
      if (debugCallback)
        debugCallback(
          `[DEBUG] Найденный deposit event: ${JSON.stringify(depositEvent, null, 2)}`
        );
      ...
      if (depositEvent?.data?.amount) {
        ...
        if (debugCallback)
          debugCallback(
            `[DEBUG] Извлеченная сумма для claim из deposit event: ${amount} ${token}`
          );
      }
    }
    ...
    if (amount === 0 && tx.events) {
      if (debugCallback)
        debugCallback(`[DEBUG] Ищем награды в других событиях...`);

      // Ищем события с наградами
      const rewardEvents = tx.events.filter(
        (event: any) =>
      ...
      if (debugCallback)
        debugCallback(
          `[DEBUG] Найдено ${rewardEvents.length} событий с наградами: ${JSON.stringify(rewardEvents, null, 2)}`
        );
      ...
      for (const event of rewardEvents) {
        if (
          ...
          const rewardAmount =
            event.data.amount || event.data.value || event.data.reward_amount;
          amount = parseFloat(rewardAmount) / Math.pow(10, decimals);
          if (debugCallback)
            debugCallback(
              `[DEBUG] Извлеченная сумма для claim из reward event: ${amount} ${token}`
            );
          break;
        }
      }
      ...
      if (amount === 0 && tx.changes) {
        if (debugCallback)
          debugCallback(`[DEBUG] Ищем награды в изменениях состояния...`);
        ...
        if (debugCallback)
          debugCallback(
            `[DEBUG] Найдено ${coinStoreChanges.length} изменений CoinStore: ${JSON.stringify(coinStoreChanges, null, 2)}`
          );
        ...
        for (const change of coinStoreChanges) {
          if (change.data?.data?.coin?.value) {
            ...
            if (coinValue > 0) {
              ...
              if (debugCallback)
                debugCallback(
                  `[DEBUG] Извлеченная сумма для claim из CoinStore change: ${amount} ${token}`
                );
              break;
            }
          }
        ...
        if (amount === 0) {
          ...
          if (debugCallback)
            debugCallback(
              `[DEBUG] Найдено ${fungibleStoreChanges.length} изменений FungibleStore: ${JSON.stringify(fungibleStoreChanges, null, 2)}`
            );
          ...
          for (const change of fungibleStoreChanges) {
            if (change.data?.data?.balance) {
              ...
              if (balanceValue > 0) {
                ...
                if (debugCallback)
                  debugCallback(
                    `[DEBUG] Извлеченная сумма для claim из FungibleStore change: ${amount} ${token}`
                  );
                break;
              }
            }
      ...
      if (amount === 0 && tx.payload?.arguments) {
        if (debugCallback)
          debugCallback(`[DEBUG] Ищем награды в аргументах транзакции...`);
        ...
        for (const arg of tx.payload.arguments) {
          if (
            ...
            if (debugCallback)
              debugCallback(
                `[DEBUG] Извлеченная сумма для claim из аргумента: ${amount} ${token}`
              );
            break;
          }
        }
    ...
    if (amount === 0) {
      if (debugCallback)
        debugCallback(
          `[DEBUG] Не удалось извлечь сумму наград из claim операции`
        );
      if (debugCallback)
        debugCallback(
          `[DEBUG] Полная структура транзакции: ${JSON.stringify(tx, null, 2)}`
        );
    }
  } else if (
    functionName.includes('liquidate') ||
    functionName.includes('flash_loan') ||
    functionName.includes('fee')
  ) {
    // Операции с комиссиями
    if (debugCallback)
      debugCallback(
        `[DEBUG] Обрабатываем операцию с комиссиями: ${functionName}`
      );

    // Ищем комиссии в событиях
    if (tx.events) {
      const feeEvents = tx.events.filter(
        (event: any) =>
      ...
      if (debugCallback)
        debugCallback(
          `[DEBUG] Найдено ${feeEvents.length} событий с комиссиями: ${JSON.stringify(feeEvents, null, 2)}`
        );

      for (const event of feeEvents) {
        if (event.data?.amount || event.data?.fee || event.data?.commission) {
          const feeAmount =
            event.data.amount || event.data.fee || event.data.commission;
          amount = parseFloat(feeAmount) / Math.pow(10, decimals);
          if (debugCallback)
            debugCallback(
              `[DEBUG] Извлеченная комиссия из события: ${amount} ${token}`
            );
          break;
        }
      }
    ...
    if (amount === 0 && tx.changes) {
      const feeChanges = tx.changes.filter(
        (change: any) =>
      ...
      for (const change of feeChanges) {
        if (change.data?.data?.amount || change.data?.data?.fee) {
          ...
          if (debugCallback)
            debugCallback(
              `[DEBUG] Извлеченная комиссия из изменений состояния: ${amount} ${token}`
            );
          break;
        }
      }
  ...
  if (debugCallback)
    debugCallback(
      `[DEBUG] extractEchelonAmount возвращает: ${amount} ${token} (decimals: ${decimals})`
    );
  if (debugCallback)
    debugCallback(
      `[DEBUG] Финальный токен: ${token}, сумма: ${amount}, decimals: ${decimals}`
    );
  if (debugCallback)
    debugCallback('[DEBUG] === КОНЕЦ extractEchelonAmount ===');
  return { amount, token, decimals };
}
...
function extractJouleAmount(
  ...
  if (!tx.events || !Array.isArray(tx.events)) {
    console.log(
      '[DEBUG] extractJouleAmount - нет событий, возвращаем значения по умолчанию'
    );
    return { amount, token, decimals };
  }
  ...
  const coinEvents = tx.events.filter(
    (event: any) =>
      event.type?.includes('coin::') || event.type?.includes('CoinStore')
  );

  if (functionName.includes('deposit') || functionName.includes('supply')) {
    // Депозит в Joule
    const withdrawEvent = coinEvents.find(
      (event: any) =>
    ...
    // Вывод из Joule
    const depositEvent = coinEvents.find(
      (event: any) =>
  ...
  console.log('[DEBUG] extractJouleAmount возвращает:', {
    amount,
    token,
    decimals,
  });
 ...
 */
function extractHyperionAmount(
  tx: any,
  userAddress: string
): { amount: number; token: string; decimals: number } {
  ...
  if (
    ...
    console.log(
      '[DEBUG] 2. Проверяем payload type_arguments:',
      tx.payload?.type_arguments
    );
    console.log(
      '[DEBUG] 3. Проверяем payload arguments:',
  ...
  if (tx.changes) {
    ...
    for (const change of fungibleStoreChanges) {
      if (
        change.data?.type?.includes('Metadata') &&
        change.data?.data?.symbol
      ) {
    ...
    if (token === 'APT') {
      for (const change of fungibleStoreChanges) {
        if (
          change.data?.type?.includes('FungibleStore') &&
          change.data?.data?.metadata?.inner
        ) {
          const tokenAddress = change.data.data.metadata.inner;
          console.log(
            '[DEBUG] Найден адрес токена в FungibleStore:',
            tokenAddress
          );
          ...
          if (tokenInfo) {
            ...
            decimals = tokenInfo.decimals;
            console.log(
              '[DEBUG] Определен токен из FungibleStore:',
              token,
              decimals
            );
            break;
      ...
      tx.events.forEach((event: any, index: number) => {
        ...
        if (event.type && event.type.includes('<')) {
          ...
          if (typeMatch) {
            ...
            if (
              tokenType.includes('stapt::StakedApt') ||
              tokenType.includes('stapt_token::StakedApt')
            ) {
              token = 'stAPT';
              decimals = getTokenDecimals('stAPT');
      ...
      if (tx.payload?.arguments && tx.payload.arguments.length > 0) {
        ...
        tx.payload.arguments.forEach((arg: any, index: number) => {
          ...
          if (typeof arg === 'string' && arg.includes('0x')) {
            ...
            if (tokenInfo) {
              console.log(
                '[DEBUG] Найден токен в аргументах:',
                tokenInfo.symbol
              );
            }
          }
  ...
  if (functionName.includes('remove_liquidity')) {
    ...
    const depositEvents = tx.events.filter(
      (event: any) => event.type === '0x1::fungible_asset::Deposit'
    );
    ...
    if (amount === 0) {
      for (const depositEvent of depositEvents) {
        if (
          depositEvent.data?.amount &&
          depositEvent.data?.store?.includes(userAddress)
        ) {
          amount = parseFloat(depositEvent.data.amount);
          amount = amount / Math.pow(10, decimals);
          break;
    ...
    const withdrawEvents = tx.events.filter(
      (event: any) => event.type === '0x1::fungible_asset::Withdraw'
    );
    ...
    if (amount === 0) {
      for (const withdrawEvent of withdrawEvents) {
        if (
          withdrawEvent.data?.amount &&
          withdrawEvent.data?.store?.includes(userAddress)
        ) {
          amount = parseFloat(withdrawEvent.data.amount);
          amount = amount / Math.pow(10, decimals);
          break;
    ...
    if (
      ...
      console.log(
        '[DEBUG] Все изменения:',
        JSON.stringify(tx.changes, null, 2)
      );
      console.log('[DEBUG] Payload:', JSON.stringify(tx.payload, null, 2));
    }
    ...
    if (swapEvents.length > 0) {
      ...
      if (swapEvent.data?.token_in) {
        ...
        if (tokenIn.inner) {
          ...
          if (tokenInfo) {
            ...
            decimals = tokenInfo.decimals;
            console.log(
              '[DEBUG] Определен токен из token_in:',
              token,
              decimals
            );
          }
      ...
      if (
        ...
        console.log(
          '[DEBUG] SwapEvent data детально:',
          JSON.stringify(swapEvent.data, null, 2)
        );
  ...
  console.log('[DEBUG] extractHyperionAmount возвращает:', {
    amount,
    token,
    decimals,
  });
...
function extractGenericAmount(
  ...
  if (!tx.events || !Array.isArray(tx.events)) {
    console.log(
      '[DEBUG] extractGenericAmount - нет событий, возвращаем значения по умолчанию'
    );
    return { amount, token, decimals };
  }
      ...
      if (event.data) {
        ...
        allKeys.forEach(key => {
          ...
          if (
            ...
            console.log(
              `[DEBUG] Найдено числовое значение в событии ${index}, ключ ${key}:`,
              value
            );
          }
        });
        ...
        if (change.data?.data) {
          ...
          allKeys.forEach(key => {
            ...
            if (
              ...
              console.log(
                `[DEBUG] Найдено числовое значение в изменении ${index}, ключ ${key}:`,
                value
              );
            }
          });
  ...
  const amountEvents = tx.events.filter(
    (event: any) =>
      event.data?.amount || event.data?.value || event.data?.coin_amount
  );
  ...
  if (
    ...
    console.log(
      '[DEBUG] Не найдено событий с суммой, анализируем все события...'
    );
    ...
    if (tx.payload?.type_arguments && tx.payload.type_arguments.length > 0) {
      ...
      tx.payload.type_arguments.forEach((typeArg: string, index: number) => {
        ...
        if (
          ...
          decimals = getTokenDecimals('stAPT');
          console.log(
            '[DEBUG] Определен stAPT из type_arguments в extractGenericAmount'
          );
        }
      });
    }
    ...
    if (
      tx.payload?.function &&
      (tx.payload.function.includes('stapt') ||
        tx.payload.function.includes('StakedApt'))
    ) {
      token = 'stAPT';
      decimals = getTokenDecimals('stAPT');
      console.log(
        '[DEBUG] Определен stAPT из function name в extractGenericAmount'
      );
    }
  }
  ...
  if (
    (tx.payload?.function && tx.payload.function.includes('swap')) ||
    (tx.payload?.function === undefined &&
    ...
    if (!tx.payload?.function) {
      console.log(
        '[DEBUG] Функция не определена, анализируем события для определения типа операции...'
      );

      const hasSwapEvents = tx.events.some(
        (event: any) =>
      ...
      if (!hasSwapEvents) {
        console.log(
          '[DEBUG] Не найдены swap события, возвращаем значения по умолчанию'
        );
        return { amount, token, decimals };
      }
    }
    ...
    // Ищем события swap
    const swapEvents = tx.events.filter(
      (event: any) =>
        event.type.includes('swap') ||
        event.type.includes('Swap') ||
        event.type.includes('trade') ||
    ...
    if (amount === 0) {
      console.log(
        '[DEBUG] Не найдена сумма в swap событиях, ищем в других событиях...'
      );
          ...
          if (
            ...
            if (change.data?.data?.coin?.value) {
              ...
              if (!isNaN(coinValue) && coinValue > 0) {
                console.log(
                  '[DEBUG] Найдена сумма в CoinStore change:',
                  coinValue
                );
                amount = coinValue / Math.pow(10, decimals);
              }
        ...
        if (event.type && event.type.includes('<')) {
          ...
          if (typeMatch) {
            ...
            if (
              ...
              decimals = getTokenDecimals('stAPT');
              console.log(
                '[DEBUG] Определен stAPT из типа события в extractGenericAmount'
              );
            }
          }
        }
  ...
  console.log('[DEBUG] extractGenericAmount возвращает:', {
    amount,
    token,
    decimals,
  });
 ...
 */
export function getTokenInfoByCoinName(
  coinName: string,
  debugCallback?: (message: string) => void
  ...
  if (debugCallback)
    debugCallback(`[DEBUG] getTokenInfoByCoinName вызвана с: ${coinName}`);

  // Специальная обработка для известных адресов stAPT
  if (
    coinName.includes('stapt') ||
    coinName.includes('StakedApt') ||
    coinName.includes(
      '0xb614bfdf9edc39b330bbf9c3c5bcd0473eee2f6d4e21748629cc367869ece627'
    )
  ) {
    if (debugCallback)
      debugCallback(`[DEBUG] Обнаружен stAPT адрес: ${coinName}`);
  ...
  if (debugCallback)
    debugCallback(`[DEBUG] Нормализованное имя: ${normalizedCoinName}`);
  ...
  if (token) {
    if (debugCallback)
      debugCallback(`[DEBUG] Найден токен по faAddress: ${token.symbol}`);
    return {
      name: token.name,
      symbol: token.symbol,
  ...
  if (token) {
    if (debugCallback)
      debugCallback(`[DEBUG] Найден токен по tokenAddress: ${token.symbol}`);
    return {
      name: token.name,
      symbol: token.symbol,
      decimals: token.decimals,
  ...
  if (!token) {
    token = tokenList.data.data.find((token: any) => {
      return (
        token.name === normalizedCoinName || token.symbol === normalizedCoinName
      );
    });
  }
  ...
  if (!token) {
    token = tokenList.data.data.find((token: any) => {
      return (
        token.name.toLowerCase().includes(normalizedCoinName.toLowerCase()) ||
  ...
  if (token) {
    if (debugCallback)
      debugCallback(
        `[DEBUG] Найден токен по частичному совпадению: ${token.symbol}`
      );
    return {
      name: token.name,
      symbol: token.symbol,
  ...
  if (!token) {
    if (debugCallback)
      debugCallback(
        `[DEBUG] Пробуем специальную обработку для: ${normalizedCoinName}`
      );
    // Проверяем известные FA адреса
    if (normalizedCoinName === '0xa') {
      if (debugCallback)
        debugCallback('[DEBUG] Найден APT по специальной обработке');
    ...
    if (
      ...
      if (debugCallback)
        debugCallback('[DEBUG] Найден stAPT по специальной обработке');
      return { name: 'Staked Aptos Coin', symbol: 'stAPT', decimals: 8 };
    }
    if (
      normalizedCoinName ===
      ...
      if (debugCallback)
        debugCallback('[DEBUG] Найден USDt по специальной обработке');
    ...
    if (
      ...
      if (debugCallback)
        debugCallback('[DEBUG] Найден USDC по специальной обработке');
    ...
    if (
      ...
      if (debugCallback)
        debugCallback('[DEBUG] Найден thAPT по специальной обработке');
    ...
    if (
      ...
      if (debugCallback)
        debugCallback('[DEBUG] Найден sthAPT по специальной обработке');
 ...
 */
function determineTokenFromEvent(
  event: any,
  ...
  if (debugCallback)
    debugCallback(
      `[DEBUG] determineTokenFromEvent вызвана с: ${JSON.stringify({ eventType: event.type, typeArguments })}`
    );
  ...
  if (event.type) {
    ...
    if (typeMatch) {
      const tokenType = typeMatch[1];
      if (debugCallback)
        debugCallback(`[DEBUG] Найден тип токена в событии: ${tokenType}`);

      if (tokenType.includes('aptos_coin::AptosCoin')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен APT из типа события');
        return 'APT';
      }
      if (tokenType.includes('usda::USDA')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен USDA из типа события');
        return 'USDA';
      }
      if (
        tokenType.includes('stapt::StakedApt') ||
        ...
        if (debugCallback)
          debugCallback('[DEBUG] Определен stAPT из типа события');
        return 'stAPT';
      }
      if (tokenType.includes('usde::USDe')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен USDe из типа события');
        return 'USDe';
      }
      if (tokenType.includes('staked_usde::StakedUSDe')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен sUSDe из типа события');
        return 'sUSDe';
      }
      if (tokenType.includes('staking::ThalaAPT')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен thAPT из типа события');
        return 'thAPT';
      }
      if (tokenType.includes('staking::StakedThalaAPT')) {
        if (debugCallback)
          debugCallback('[DEBUG] Определен sthAPT из типа события');
        return 'sthAPT';
      }
  ...
  if (debugCallback)
    debugCallback(
      '[DEBUG] determineTokenFromEvent возвращает APT по умолчанию'
    );
...
function getTokenDecimals(
  ...
  if (debugCallback)
    debugCallback(`[DEBUG] getTokenDecimals вызвана с: ${token}`);
  ...
  if (debugCallback)
    debugCallback(`[DEBUG] getTokenDecimals возвращает: ${decimals}`);
  return decimals;
}
...
export function calculateTotalProfit(transactions: Transaction[]): {
  ...
  const averageROI =
    protocolBreakdown.length > 0
      ? protocolBreakdown.reduce((sum, p) => sum + p.profitData.roi, 0) /
        protocolBreakdown.length
      : 0;

  const mostProfitableProtocol =
    protocolBreakdown.length > 0 ? protocolBreakdown[0].protocolName : '';
  const leastProfitableProtocol =
    protocolBreakdown.length > 0
      ? protocolBreakdown[protocolBreakdown.length - 1].protocolName
      : '';
 ...
 */
export function formatProfitValue(
  value: number,
  currency: string = 'APT'
): string {
