    ...
    persist(
      (set, get) => ({
        ...
        fetchPositions: async (address: string) => {
          ...
          try {
            console.log(
              '[HyperionStore] Fetching positions for address:',
              address
            );

            const response = await fetch(
              `/api/protocols/hyperion/userPositions?address=${address}`
            );
            ...
            if (data.success && Array.isArray(data.data)) {
              ...
              const uniquePositions = data.data.filter(
              ...
              console.log(
                '[HyperionStore] Positions fetched successfully:',
        ...
        fetchPools: async () => {
          ...
          try {
            ...
            if (data.success && Array.isArray(data.data)) {
              ...
              console.log(
                '[HyperionStore] Pools fetched successfully:',
                data.data.length
              );
            } else {
              set({
        ...
        fetchPrices: async (tokenAddresses: string[], forceRefresh = false) => {
          ...
          try {
            console.log(
              '[HyperionStore] Fetching prices for tokens:',
              tokenAddresses.length
            );

            // Use Panora API to fetch prices
            const { PanoraPricesService } = await import(
              '@/lib/services/panora/prices'
            );
            ...
            for (const address of tokenAddresses) {
              ...
              try {
                const priceData = await pricesService.getPrices(1, [
                  cleanAddress,
                ]);
                ...
                if (
                  ...
                  const token = priceData.data.find(
                    (t: any) =>
                      t.tokenAddress === cleanAddress ||
                      t.faAddress === cleanAddress
                  );

                  if (token && token.usdPrice) {
                    prices[address] = token.usdPrice;
                    console.log(
                      '[HyperionStore] Added price for',
                      address,
                      ':',
                      token.usdPrice
                    );
                ...
                console.warn(
                  '[HyperionStore] Failed to fetch price for',
                  address,
                  error
                );
              }
            ...
            console.log(
              '[HyperionStore] Current state prices before update:',
              state.prices
            );

            const newPrices = { ...state.prices, ...prices };
            console.log(
              '[HyperionStore] New prices object after merge:',
              newPrices
            );
            ...
            console.log(
              '[HyperionStore] State prices after update:',
              updatedState.prices
            );
            console.log(
              '[HyperionStore] Prices fetched and saved successfully'
            );
          } catch (error) {
        ...
        getPosition: (positionId: string) => {
          ...
          return state.positions.find(
            pos => pos.position.objectId === positionId
          );
        },
        ...
        isDataStale: (type: 'positions' | 'pools' | 'prices') => {
          ...
          const lastUpdate = state[
            `last${type.charAt(0).toUpperCase() + type.slice(1)}Update` as keyof HyperionState
          ] as number | null;
