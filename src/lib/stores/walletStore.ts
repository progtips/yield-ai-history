...
interface WalletState {
  ...
  clearData: () => void;
  isDataStale: (
    type: 'balance' | 'positions' | 'rewards' | 'prices'
  ) => boolean;
    ...
    persist(
      (set, get) => ({
        ...
        fetchBalance: async (address: string, forceRefresh = false) => {
          ...
          try {
            ...
            const response = await fetch(
              `/api/aptos/walletBalance?address=${encodeURIComponent(address)}`
            );
            ...
            if (data.balances) {
              ...
              console.log(
                '[WalletStore] Balance fetched successfully:',
        ...
        fetchPositions: async (
          ...
          try {
            console.log(
              '[WalletStore] Fetching positions for address:',
              address
            );
            ...
            const promises = protocolsToFetch.map(async protocol => {
              try {
                const response = await fetch(
                  `/api/protocols/${protocol}/userPositions?address=${encodeURIComponent(address)}`
                );
                ...
                if (response.ok) {
                  ...
                  console.log(
                    `[WalletStore] ${protocol} positions fetched:`,
                    newPositions[protocol].length
                  );
                } else {
                  console.warn(
                    `[WalletStore] Failed to fetch ${protocol} positions:`,
                    response.status
                  );
                  newPositions[protocol] = [];
                }
                ...
                console.error(
                  `[WalletStore] Error fetching ${protocol} positions:`,
                  error
                );
                newPositions[protocol] = [];
              }
        ...
        fetchRewards: async (
          ...
          try {
            ...
            const promises = protocolsToFetch.map(async protocol => {
              try {
                const response = await fetch(
                  `/api/protocols/${protocol}/rewards?address=${encodeURIComponent(address)}`
                );
                ...
                if (response.ok) {
                  ...
                  console.log(
                    `[WalletStore] ${protocol} rewards fetched:`,
                    newRewards[protocol].length
                  );
                } else {
                  console.warn(
                    `[WalletStore] Failed to fetch ${protocol} rewards:`,
                    response.status
                  );
                  newRewards[protocol] = [];
                }
                ...
                console.error(
                  `[WalletStore] Error fetching ${protocol} rewards:`,
                  error
                );
                newRewards[protocol] = [];
              }
        ...
        fetchPrices: async (tokenAddresses: string[], forceRefresh = false) => {
          ...
          try {
            console.log(
              '[WalletStore] Fetching prices for tokens:',
        ...
        isDataStale: (type: 'balance' | 'positions' | 'rewards' | 'prices') => {
          ...
          const lastUpdate = state[
            `last${type.charAt(0).toUpperCase() + type.slice(1)}Update` as keyof WalletState
          ] as number | null;
