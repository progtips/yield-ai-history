      ...
      (set, get) => ({
        ...
        fetchPositions: async (address: string) => {
          ...
          try {
            ...
            const response = await fetch(
              `/api/protocols/auro/userPositions?address=${encodeURIComponent(address)}`
            );
            const data = await response.json();

            if (response.ok && data.success) {
              const positions = Array.isArray(data.positionInfo)
                ? data.positionInfo
                : [];
              set({
                positions,
              ...
              console.log(
                '[AuroStore] Positions fetched successfully:',
                positions.length
              );
            } else {
        ...
        fetchRewards: async (address: string) => {
          ...
          if (state.positions.length === 0 || state.pools.length === 0) {
            console.log(
              '[AuroStore] No positions or pools data available for rewards'
            );
            return;
          }
          ...
          try {
            console.log(
              '[AuroStore] Fetching rewards for positions:',
        ...
        fetchPools: async () => {
          ...
          try {
            ...
            if (data.success && Array.isArray(data.data)) {
              ...
              console.log(
                '[AuroStore] Pools fetched successfully:',
                data.data.length
              );
            } else {
        ...
        fetchPrices: async (tokenAddresses: string[], forceRefresh = false) => {
          ...
          try {
            console.log(
              '[AuroStore] Fetching prices for tokens:',
            ...
            console.log(
              '[AuroStore] Current state prices before update:',
              state.prices
            );

            const newPrices = { ...state.prices, ...prices };
            console.log(
              '[AuroStore] New prices object after merge:',
              newPrices
            );
            ...
            console.log(
              '[AuroStore] State prices after update:',
              updatedState.prices
            );
        ...
        isDataStale: (maxAgeMs = CACHE_TTL.POSITIONS) => {
          ...
          return (
            !state.lastPositionsUpdate ||
            now - state.lastPositionsUpdate > maxAgeMs
          );
        },
      }),
      {
