...
export function useDeposit() {
  ...
  const deposit = useCallback(
    async (protocolKey: ProtocolKey, token: string, amount: bigint) => {
      try {
        ...
        console.log(
          ...
          Object.getOwnPropertyNames(Object.getPrototypeOf(protocolInstance))
        );
        ...
        if (typeof protocolInstance.buildDeposit !== 'function') {
          throw new Error(
            `Protocol ${protocolKey} does not have buildDeposit method`
          );
        }
        ...
        if (response.hash) {
          ...
          for (let i = 0; i < maxAttempts; i++) {
            console.log(
              `Checking transaction status attempt ${i + 1}/${maxAttempts}`
            );
            try {
              ...
              if (
                ...
                console.log(
                  'Transaction confirmed successfully, showing toast...'
                );
