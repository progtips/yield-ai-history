...
export function useTransactionSubmitter() {
  ...
  const submitTransaction = useCallback(
    async (request: TransactionRequest) => {
      ...
      const aptBalance = await AptBalanceService.getAptBalance(
      ...
      if (aptBalance > 0) {
        ...
        const response = await wallet.signAndSubmitTransaction({
          ...
          options: {
            maxGasAmount: Math.min(
              request.options?.maxGasAmount || 20000,
              20000
            ), // Network limit is 20000
          },
        });
        ...
        if (!gasStationService.isAvailable()) {
          throw new Error(
            'Gas station is not available. Please ensure you have APT for gas fees or configure gas station.'
          );
        }
        ...
        const response = await wallet.signAndSubmitTransaction({
          ...
          options: {
            maxGasAmount: Math.min(
              request.options?.maxGasAmount || 20000,
              20000
            ), // Gas station limit is 20000
          },
          withFeePayer: true, // Enable gas station
        });

        console.log(
          'Gas station transaction submitted successfully:',
          response
        );
        return response;
      }
