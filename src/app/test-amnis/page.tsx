...
export default function TestAmnisPage() {
  ...
  const fetchStakedAmounts = async () => {
    ...
    try {
      ...
      for (const pool of stakingPools) {
        try {
          const response = await fetch(
            `/api/protocols/amnis/staking-amount?userAddress=${walletAddress}&poolAddress=${pool.address}`
          );
          const data = await response.json();
          ...
          if (data.success) {
            ...
            console.error(
              'Failed to get staked amount for pool:',
              pool.address,
              data.error
            );
          ...
          console.error(
            'Error getting staked amount for pool:',
            pool.address,
            error
          );
          amounts[pool.address] = 0;
  ...
  const fetchAmiPrice = async () => {
    ...
    try {
      ...
      const amiTokenAddress =
        '0xb36527754eb54d7ff55daf13bcb54b42b88ec484bd6f0e3b2e0d1db169de6451';

      const response = await fetch(
        `/api/panora/tokenPrices?chainId=1&tokenAddress=${amiTokenAddress}`
      );
      const data = await response.json();

      if (data.data && Array.isArray(data.data) && data.data.length > 0) {
        const amiToken = data.data.find(
          (token: any) =>
            token.tokenAddress === amiTokenAddress ||
            token.faAddress === amiTokenAddress
        );
    ...
    try {
      ...
      if (testToken === '0x1::aptos_coin::AptosCoin') {
        const expectedPayload = {
          ...
          type_arguments: [],
          arguments: [testAmount, walletAddress],
        };
        ...
        alert(
          'Deposit payload generated successfully! Check console for details.'
        );
      }
    } catch (error) {
  ...
  const testWithdraw = async () => {
    try {
      ...
      alert(
        'Withdraw payload generated successfully! Check console for details.'
      );
    } catch (error) {
  ...
  const testPoolsIntegration = async () => {
    try {
      ...
      // Filter Amnis pools
      const amnisPools =
        data.data?.filter((pool: any) => pool.protocol === 'Amnis Finance') ||
        [];
      console.log('Amnis pools in main pools API:', amnisPools);

      alert(
        `Found ${amnisPools.length} Amnis pools in main pools API! Check console for details.`
      );
    } catch (error) {
    ...
    testCases.forEach(testCase => {
      ...
      const payload = {
        ...
        function:
          '0x111ae3e5bc816a5e63c2da97d0aa3886519e0cd5e4b046659fa35796bd11542a::router::deposit_and_stake_entry',
        type_arguments: [],
        arguments: [testCase.amount.toString(), testCase.address],
      };
  ...
  const testRealDeposit = async () => {
    ...
    try {
      ...
      console.log(
        'Payload arguments types:',
        payload.arguments.map((arg: any) => ({ value: arg, type: typeof arg }))
      );
      ...
      if (
        typeof payload.arguments[0] !== 'string' ||
        typeof payload.arguments[1] !== 'string'
      ) {
        console.error('Arguments are not strings:', {
          arg0: {
            value: payload.arguments[0],
            type: typeof payload.arguments[0],
          },
          arg1: {
            value: payload.arguments[1],
            type: typeof payload.arguments[1],
          },
        });
      ...
      console.log(
        '✅ Payload format is correct, proceeding with transaction...'
      );
      ...
      alert(
        `❌ Real deposit test failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
    ...
    try {
      ...
      if (
        ...
        const expectedPayload = {
          ...
          type_arguments: [],
          arguments: [testAmAptAmount, walletAddress],
        };
        ...
        if (
          ...
          if (
            ...
            console.log('✅ Arguments are strings (not serialized bytes)');
            alert(
              `✅ amAPT Deposit payload generated successfully!\n\nFunction: ${data.function}\nArguments: [${data.arguments.join(', ')}]\n\nArguments are in correct string format.`
            );
          } else {
            console.log('❌ Arguments are not strings:', data.arguments);
            alert(
        ...
        alert(
          'amAPT Deposit payload generated successfully! Check console for details.'
        );
      }
    } catch (error) {
  ...
  const testRealAmAptDeposit = async () => {
    ...
    try {
      ...
      alert(
        `❌ Real amAPT deposit test failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
      ...
      <div className='flex items-center gap-4'>
        ...
        <div>
          ...
          <p className='text-muted-foreground'>
            Test the Amnis Finance protocol integration
          </p>
        </div>
      </div>
          ...
          <CardContent>
            {apiData ? (
              <div className='space-y-2 text-sm'>
                ...
                <div>
                  <span className='font-medium'>amAPT Price:</span> $
                  {apiData.amAptPrice}
                </div>
                <div>
                  <span className='font-medium'>stAPT Total Supply:</span>{' '}
                  {apiData.stAptTotalSupply?.toLocaleString()}
                </div>
                <div>
                  <span className='font-medium'>amAPT Total Supply:</span>{' '}
                  {apiData.amAptTotalSupply?.toLocaleString()}
                </div>
                <div>
                  <span className='font-medium'>Stakers:</span>{' '}
                  {apiData.staker?.toLocaleString()}
                </div>
                <div>
                  <span className='font-medium'>Liquid Rate:</span>{' '}
                  {apiData.liquidRate}
                </div>
                <div>
                  <span className='font-medium'>Pancake Rate:</span>{' '}
                  {apiData.pancakeRate}
                </div>
                <div>
                  <span className='font-medium'>Panora Rate:</span>{' '}
                  {apiData.panoraRate}
                </div>
                <div>
                  <span className='font-medium'>Cellana Rate:</span>{' '}
                  {apiData.cellanaRate}
                </div>
              </div>
            ) : (
      ...
      <Card>
        ...
        <CardContent>
          <div className='flex gap-2 mb-4'>
            ...
            {account?.address && (
              <Button
                onClick={fetchStakedAmounts}
                disabled={stakedAmountsLoading}
                variant='outline'
              >
            ...
            <Button
              onClick={fetchAmiPrice}
              disabled={amiPriceLoading}
              variant='outline'
            >
                ...
                <div className='grid grid-cols-2 md:grid-cols-5 gap-4 text-sm'>
                  ...
                  <div>
                    ...
                    <div className='font-semibold'>
                      {pool.lockDurationDays > 0
                        ? `${pool.lockDurationDays} days`
                        : 'No lock'}
                    </div>
                  </div>
                  <div>
                        ...
                        <div>
                          ...
                          {amiPrice > 0 && (
                            <div className='text-xs text-green-600'>
                              $
                              {(
                                (stakedAmounts[pool.address] || 0) * amiPrice
                              ).toFixed(2)}
                            </div>
                          )}
                        </div>
                ...
                {pool.luckyWheelRate && (
                  <div className='mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded'>
                    <span className='font-medium text-yellow-800'>
                      Lucky Wheel Rate: {pool.luckyWheelRate}%
                    </span>
                  </div>
                )}
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  ...
                  <p className='text-sm text-gray-600'>
                    {testAmount} ({testAmount / 100000000} APT)
                  </p>
                </div>
                <div>
                  <label className='block text-sm font-medium mb-1'>
                  ...
                  <p className='text-sm text-gray-600'>
                    {testAmAptAmount} ({testAmAptAmount / 100000000} amAPT)
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <p className='text-sm text-muted-foreground'>
                Pioneering Liquid Staking protocol on Aptos
              </p>
            ...
            <div>
              ...
              <p className='text-sm text-green-600'>
                ✅ Management interface ready
              </p>
              <p className='text-sm text-green-600'>
                ✅ Pools integration added
              </p>
              <p className='text-sm text-green-600'>
