...
export default function TestHyperionProfitPage() {
  ...
  const [hyperionProfitResults, setHyperionProfitResults] = React.useState<
    any[]
  >([]);
  ...
  const addDebugInfo = React.useCallback(
    (message: string) => {
      if (isClient) {
  ...
  React.useEffect(() => {
    if (isClient) {
      ...
      const testToken3 = getTokenInfoByCoinName(
      ...
      // Тестируем stAPT
      const testStAPT = getTokenInfoByCoinName(
      ...
      addDebugInfo(
        `Тест для "Staked Aptos Coin": ${JSON.stringify(testStAPT3)}`
      );
  ...
  function getProtocolNameByFunction(
    ...
    if (
      functionName.includes('hyperion') ||
  ...
  const calculateHyperionProfitWithData = React.useCallback(
    (transactionsData: any[]) => {
      ...
      hyperionOnly.slice(0, 3).forEach((tx, index) => {
        addDebugInfo(
          `Hyperion транзакция ${index + 1}: function=${tx.function}, type=${tx.type}`
        );
      });
      ...
      if (hyperionOnly.length === 0 && transactionsData.length > 0) {
        addDebugInfo(
          'Hyperion транзакции не найдены, анализируем все транзакции'
        );
        transactionsToAnalyze = transactionsData;
      }

      const filteredTransactions = transactionsToAnalyze.filter(
        tx =>
          tx.type === 'deposit' ||
          tx.type === 'withdraw' ||
          tx.type === 'claim' ||
      ...
      addDebugInfo(
        `Отфильтрованные транзакции deposit/withdraw/claim/swap: ${filteredTransactions.length}`
      );
      ...
      addDebugInfo(
        `Распределение типов транзакций: ${JSON.stringify(typeCounts)}`
      );
      ...
      try {
        filteredTransactions.forEach((tx, index) => {
          try {
            addDebugInfo(
              `Обрабатываем транзакцию ${index + 1}: ${tx.function} ${tx.type}`
            );
            // Извлекаем сумму и валюту из транзакции
            const { amount: extractedAmount, token: extractedToken } =
              extractTransactionAmount(tx._rawData, tx.from);

            // Дополнительная отладка для claim транзакций
            if (tx.type === 'claim') {
              addDebugInfo(
                `Claim транзакция: extractedAmount=${extractedAmount}, extractedToken=${extractedToken}`
              );
            }

            // Дополнительная отладка для всех транзакций
            addDebugInfo(
              `Транзакция ${index + 1}: extractedAmount=${extractedAmount}, extractedToken=${extractedToken}`
            );
            ...
            if (
              ...
              addDebugInfo(
                '[DEBUG] === СПЕЦИАЛЬНАЯ ОТЛАДКА ДЛЯ КОНКРЕТНОЙ ТРАНЗАКЦИИ ==='
              );
              ...
              addDebugInfo(
                `[DEBUG] Raw Data Events: ${JSON.stringify(tx._rawData?.events)}`
              );
              addDebugInfo(
                `[DEBUG] Raw Data Changes: ${JSON.stringify(tx._rawData?.changes)}`
              );
              addDebugInfo(
                `[DEBUG] Raw Data Payload: ${JSON.stringify(tx._rawData?.payload)}`
              );
            ...
            addDebugInfo(
              `[DEBUG] Транзакция ${index + 1}: function=${tx.function}, type=${tx.type}, extractedAmount=${extractedAmount}, extractedToken=${extractedToken}`
            );

            // Дополнительная отладка для claim транзакций
            if (tx.type === 'claim') {
              addDebugInfo(
                `[DEBUG] Claim транзакция ${index + 1} - Events: ${JSON.stringify(tx._rawData?.events)}`
              );
              addDebugInfo(
                `[DEBUG] Claim транзакция ${index + 1} - Changes: ${JSON.stringify(tx._rawData?.changes)}`
              );
              addDebugInfo(
                `[DEBUG] Claim транзакция ${index + 1} - Payload: ${JSON.stringify(tx._rawData?.payload)}`
              );
            }
            ...
            if (tx._rawData?.events) {
              ...
              if (tx._rawData?.changes) {
                ...
                if (poolChange?.data?.data?.asset_name) {
                  ...
                  addDebugInfo(
                    `Token info found: ${JSON.stringify(tokenInfo)}`
                  );
                  if (tokenInfo) {
                    actualToken = tokenInfo.symbol;
                    tokenDecimals = tokenInfo.decimals;
                    addDebugInfo(
                      `Using token: ${actualToken}, with decimals: ${tokenDecimals}`
                    );
                  }
                }
              }
              ...
              if (swapEventV3 && swapEventV3.data) {
                ...
                actualAmount =
                  parseFloat(swapEventV3.data.amount_in) / Math.pow(10, 6); // USDC has 6 decimals
                actualToken = 'USDC'; // Из события видно, что это USDC -> USDt swap
                addDebugInfo(
                  `SwapEventV3 found: amount_in=${swapEventV3.data.amount_in}, calculated=${actualAmount}`
                );
              } else if (swapEvent && swapEvent.data) {
                actualAmount =
                  parseFloat(swapEvent.data.amount) /
                  Math.pow(10, tokenDecimals);
              } else if (liquidityEvent && liquidityEvent.data) {
                actualAmount =
                  parseFloat(liquidityEvent.data.amount) /
                  Math.pow(10, tokenDecimals);
              }
            }
            ...
            if (tx.type === 'deposit') {
              ...
              addDebugInfo(
                `Обрабатываем claim: amount=${actualAmount}, signedAmount=${signedAmount}, token=${actualToken}`
              );
              ...
              if (tx._rawData?.events) {
                ...
                if (swapEventV3 && swapEventV3.data) {
                  ...
                  if (fromToken && toToken) {
                    const amountIn =
                      parseFloat(swapEventV3.data.amount_in) / Math.pow(10, 6);
                    const amountOut =
                      parseFloat(swapEventV3.data.amount_out) / Math.pow(10, 6);
                    ...
                    addDebugInfo(
                      `Swap direction: ${amountIn} USDC -> ${amountOut} USDt, signedAmount=${signedAmount}`
                    );
                  }
                } else {
              ...
              addDebugInfo(
                `Обрабатываем swap: amount=${actualAmount}, signedAmount=${signedAmount}, token=${actualToken}`
              );
            } else if (tx.type === 'fee') {
              operationType = 'fee';
              signedAmount = -Math.abs(actualAmount); // Комиссии всегда отрицательные (расход)
              addDebugInfo(
                `Обрабатываем fee: amount=${actualAmount}, signedAmount=${signedAmount}, token=${actualToken}`
              );
            }
            ...
            addDebugInfo(
              `Ошибка при обработке транзакции ${index + 1}: ${error}`
            );
        ...
        addDebugInfo(
          `Критическая ошибка в цикле обработки транзакций: ${error}`
        );
      }
      ...
      try {
        addDebugInfo(
          `Начинаем расчет прибыли для ${Object.keys(transactionsByCurrency).length} валют`
        );
        addDebugInfo(
          `Валюты: ${Object.keys(transactionsByCurrency).join(', ')}`
        );
        results = Object.entries(transactionsByCurrency).map(
          ([currency, transactions]) => {
            addDebugInfo(
              `Обрабатываем валюту ${currency}: ${transactions.length} транзакций`
            );
            ...
            addDebugInfo(
              `Найдено ${claimTransactions.length} транзакций типа claim для ${currency}`
            );
            addDebugInfo(
              `Найдено ${swapTransactions.length} транзакций типа swap для ${currency}`
            );
            addDebugInfo(
              `Найдено ${feeTransactions.length} транзакций типа fee для ${currency}`
            );
            claimTransactions.forEach((tx, index) => {
              addDebugInfo(
                `Claim транзакция ${index + 1}: amount=${tx.amount}, signedAmount=${tx.signedAmount}, function=${tx.function}`
              );

              // Дополнительная отладка для claim транзакций
              if (tx._rawData) {
                addDebugInfo(
                  `Claim ${index + 1} - Events count: ${tx._rawData.events?.length || 0}`
                );
                addDebugInfo(
                  `Claim ${index + 1} - Changes count: ${tx._rawData.changes?.length || 0}`
                );

                // Логируем события для claim
                if (tx._rawData.events) {
                  tx._rawData.events.forEach(
                    (event: any, eventIndex: number) => {
                      addDebugInfo(
                        `Claim ${index + 1} Event ${eventIndex}: type=${event.type}, data=${JSON.stringify(event.data)}`
                      );
                    }
                  );
                }
                ...
                if (tx._rawData.changes) {
                  tx._rawData.changes.forEach(
                    (change: any, changeIndex: number) => {
                      addDebugInfo(
                        `Claim ${index + 1} Change ${changeIndex}: type=${change.data?.type}, address=${change.address}`
                      );
                    }
                  );
                }
            ...
            const totalClaims = claimTransactions.reduce((sum, tx) => {
              const amount =
                parseFloat(tx.amount.toString().replace(/[^\d.-]/g, '')) || 0;
              addDebugInfo(
                `Добавляем к totalClaims: ${amount} из транзакции ${tx.tx || tx.hash || 'unknown'}`
              );
              addDebugInfo(
                `Исходное значение tx.amount: ${tx.amount}, тип: ${typeof tx.amount}`
              );
              return sum + amount;
            }, 0);
            ...
            const totalSwaps = swapTransactions.reduce((sum, tx) => {
              const amount =
                parseFloat(tx.amount.toString().replace(/[^\d.-]/g, '')) || 0;
              addDebugInfo(
                `Добавляем к totalSwaps: ${amount} из транзакции ${tx.tx || tx.hash || 'unknown'}`
              );
              return sum + amount;
            }, 0);
            ...
            const totalFees = feeTransactions.reduce((sum, tx) => {
              const amount =
                parseFloat(tx.amount.toString().replace(/[^\d.-]/g, '')) || 0;
              addDebugInfo(
                `Добавляем к totalFees: ${amount} из транзакции ${tx.tx || tx.hash || 'unknown'}`
              );
              return sum + amount;
            }, 0);

            // Рассчитываем общую плату за газ
            const totalGasFees = transactions.reduce(
              (sum, tx) => sum + (tx.gasFee || 0),
              0
            );

            const netPnL =
              totalWithdraw + totalClaims + totalSwaps - totalSupply;
            const realizedPnL = netPnL;
            const totalPnL =
              netPnL +
              params.remainingPosition +
              totalClaims -
              totalFees -
              totalGasFees;

            addDebugInfo(
              `Результаты для ${currency}: supply=${totalSupply}, withdraw=${totalWithdraw}, rewards=${totalClaims}, swaps=${totalSwaps}, fees=${totalFees}, netPnL=${netPnL}, totalPnL=${totalPnL}`
            );
      ...
      try {
        ...
        setTimeout(() => {
          ...
          addDebugInfo(
            `Состояние обновлено. Новое значение hyperionProfitResults: ${JSON.stringify(results)}`
          );
  ...
  const handleRefreshHistory = () => {
    ...
    const fetchRealTransactions = async (address: string) => {
      try {
        ...
        addDebugInfo(
          `Fetched ${data.length} transactions for Hyperion analysis`
        );
        ...
        addDebugInfo(
          `Transformed ${transformedTransactions.length} transactions for Hyperion analysis`
        );
    ...
    fetchRealTransactions(walletAddress)
      .then(realTransactions => {
        addDebugInfo(
          `Fetched ${realTransactions.length} real transactions for Hyperion analysis`
        );
        ...
        const hyperionOnly = realTransactions.filter(tx => {
          const protocol = getProtocolNameByFunction(tx.function, tx.to);
          addDebugInfo(
            `Transaction ${tx.hash}: function=${tx.function}, protocol=${protocol}`
          );
          return protocol === 'Hyperion';
        });
            ...
            <CardContent>
              <div className='space-y-4'>
                ...
                <div className='text-sm text-gray-600'>
                  Claim транзакций:{' '}
                  {
                    hyperionTransactions.filter(tx => tx.type === 'claim')
                      .length
                  }
                </div>
                <div className='text-sm text-gray-600'>
                  Deposit транзакций:{' '}
                  {
                    hyperionTransactions.filter(tx => tx.type === 'deposit')
                      .length
                  }
                </div>
                <div className='text-sm text-gray-600'>
                  Withdraw транзакций:{' '}
                  {
                    hyperionTransactions.filter(tx => tx.type === 'withdraw')
                      .length
                  }
                </div>
                <div className='text-sm text-gray-600'>
                  Swap транзакций:{' '}
                  {hyperionTransactions.filter(tx => tx.type === 'swap').length}
                </div>
                ...
                <div className='mt-4 p-4 bg-blue-50 rounded-lg'>
                  ...
                  <div className='space-y-2 text-xs'>
                    <div>
                      APT:{' '}
                      {getTokenInfoByCoinName('0xa')?.symbol || 'Не найден'}
                    </div>
                    <div>
                      stAPT:{' '}
                      {getTokenInfoByCoinName(
                  ...
                  <div className='bg-gray-100 p-3 rounded text-xs font-mono max-h-48 overflow-y-auto'>
                    {debugMessages.length === 0 ? (
                      <div className='text-gray-500'>
                        Отладочная информация появится здесь при выполнении
                        операций
                      </div>
                    ) : (
          ...
          <Card className='mt-4'>
            ...
            <CardContent>
              <div className='bg-gray-100 p-4 rounded text-sm font-mono max-h-96 overflow-y-auto'>
                ...
                {debugMessages.length === 0 ? (
                  <div className='text-gray-500'>
                    Отладочная информация появится здесь при выполнении операций
                  </div>
                ) : (
                  <div className='space-y-1'>
                  ...
                  {(() => {
                    ...
                    return allTransactions
                      ...
                      .map((tx, index) => {
                        // Определяем тип операции
                        const isDeposit =
                          tx.type === 'deposit' || tx.type === 'add_liquidity';
                        const isWithdraw =
                          tx.type === 'withdraw' ||
                              ...
                              <div className='text-right'>
                                ...
                                >
                                  {isDeposit
                                    ...
                                    : isWithdraw || isClaim
                                      ? '+'
                                      : ''}
                                  {actualAmount.toFixed(6)} {actualToken}
                                </div>
                                <div className='text-xs text-gray-500'>
                                  TX:{' '}
                                  <a
                                  ...
                                  >
                                    {(tx.hash || tx.tx).substring(0, 8)}...
                                    {(tx.hash || tx.tx).substring(
                                      (tx.hash || tx.tx).length - 6
                                    )}
                                  </a>
                                </div>
                                {/* Плата за газ */}
                                {(tx._rawData?.gas_used || tx.gasFee > 0) && (
                                  <div className='text-xs text-orange-600'>
                                    ...
                                    {tx.gasFee > 0
                                      ? tx.gasFee.toFixed(6)
                                      : parseInt(tx._rawData.gas_used) *
                                        (parseInt(
                                          tx._rawData.gas_unit_price || '100'
                                        ) /
                                          100000000)}{' '}
                                    APT
                                  </div>
                      ...
                      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                        <div className='bg-yellow-50 p-4 rounded-lg'>
                          ...
                          <div className='text-lg font-bold text-yellow-600'>
                            {result.remainingPosition.toFixed(6)}{' '}
                            {result.currency}
                          </div>
                        </div>
                      ...
                      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                        ...
                        <div className='bg-gray-50 p-3 rounded-lg'>
                          ...
                          <div className='font-medium text-blue-600'>
                            {result.totalSwaps.toFixed(6)} {result.currency}
                          </div>
                        </div>
                        ...
                        <div className='bg-gray-50 p-3 rounded-lg'>
                          ...
                          <div
                            className={`font-medium ${result.netPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {result.netPnL >= 0 ? '+' : ''}
                            {result.netPnL.toFixed(6)} {result.currency}
                          </div>
                        </div>
                        ...
                        <div className='bg-blue-50 p-3 rounded-lg'>
                          ...
                          <div
                            className={`font-medium ${result.totalPnL >= 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {result.totalPnL >= 0 ? '+' : ''}
                            {result.totalPnL.toFixed(6)} {result.currency}
                          </div>
                        </div>
                      </div>
                    </div>
