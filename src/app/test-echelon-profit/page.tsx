...
export default function TestEchelonProfitPage() {
  ...
  function getProtocolNameByFunction(
    ...
    const protocolAddresses: { [key: string]: string } = {
      c6bc659f1649553c1a3fa05d9727433dc03843baac29473c817d06d39e7621ba:
        'Echelon',
    };
      ...
      try {
        ...
        const transformedTransactions = data.map((tx: any, index: number) => {
          ...
          const { amount: extractedAmount, token: extractedToken } =
            extractTransactionAmount(tx, address, debugMessage => {
        ...
        try {
          ...
          if (tx._rawData?.events) {
            ...
            if (supplyEvent && supplyEvent.data) {
              actualAmount =
                parseFloat(supplyEvent.data.amount) /
                Math.pow(10, tokenDecimals);
            } else if (withdrawEvent && withdrawEvent.data) {
              actualAmount =
                parseFloat(withdrawEvent.data.amount) /
                Math.pow(10, tokenDecimals);
            }
          }
                ...
                <div className='space-y-4'>
                  ...
                  <div className='space-y-3'>
                    {echelonTransactions
                      ...
                      ) // Сортировка по времени (ранние сверху)
                      .map((tx, index) => {
                        const {
                        ...
                        if (tx._rawData?.events) {
                          const supplyEvent = tx._rawData.events.find(
                            (event: any) => event.type.includes('SupplyEvent')
                          );
                          const withdrawEvent = tx._rawData.events.find(
                            (event: any) => event.type.includes('WithdrawEvent')
                          );

                          // Определяем токен из изменений состояния (changes)
                          let tokenDecimals = 8; // По умолчанию для APT токенов
                          if (tx._rawData?.changes) {
                            const marketChange = tx._rawData.changes.find(
                              (change: any) =>
                                change.data?.type?.includes('lending::Market')
                            );
                            if (marketChange?.data?.data?.asset_name) {
                              const assetName =
                                marketChange.data.data.asset_name;
                              console.log(
                                'Found asset name in market change:',
                                assetName
                              );
                              // Ищем токен в списке по названию
                              const tokenInfo =
                                getTokenInfoByCoinName(assetName);
                              console.log('Token info found:', tokenInfo);
                              if (tokenInfo) {
                                actualToken = tokenInfo.symbol;
                                tokenDecimals = tokenInfo.decimals;
                          ...
                          if (supplyEvent && supplyEvent.data) {
                            ...
                            const depositEvent = tx._rawData.events.find(
                              (event: any) =>
                                event.type.includes('Deposit') &&
                                event.data?.store?.includes(tx.from)
                            );
                            if (depositEvent && depositEvent.data) {
                              actualAmount =
                                parseFloat(depositEvent.data.amount) /
                                Math.pow(10, tokenDecimals);
                            }
                          }
                        }
