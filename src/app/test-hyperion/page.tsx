...
export default function TestHyperionPage() {
    ...
    try {
      ...
      const filtered = allPools.filter((pool: any) => {
        const dailyVolume = parseFloat(
          pool.dailyVolumeUSD || pool.pool?.dailyVolumeUSD || '0'
        );
    ...
    <div className='container mx-auto p-6 space-y-6'>
      <div>
        <h1 className='text-2xl font-bold mb-2'>
          Test Hyperion Pools Integration
        </h1>
        ...
        <CardHeader>
          <CardTitle className='flex items-center gap-2'>
            ...
            <div className='flex items-center gap-2 ml-auto'>
              ...
              <Button onClick={testHyperionPools} disabled={loading} size='sm'>
        ...
        <CardContent>
          ...
          <div className='mb-4 p-3 bg-blue-50 rounded'>
            <div className='text-sm'>
              <strong>Filter:</strong> Showing pools with daily volume &gt;{' '}
              {formatNumber(volumeThreshold)}
            </div>
            <div className='text-sm text-gray-600'>
              Total pools: {hyperionPools.length} | Filtered pools:{' '}
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            <div>
              ...
              <div className='text-sm text-muted-foreground mb-2'>
                Pools with volume &gt; {formatNumber(volumeThreshold)}:{' '}
                {filteredPools.length}
              </div>
                ...
                return (
                  <div key={index} className='border p-3 rounded mb-3'>
                    ...
                    <div className='text-xs space-y-1'>
                      <div>
                        Pool ID:{' '}
                        <span className='font-mono'>{pool.id || p.poolId}</span>
                      </div>
                      <div>
                        Fee APR:{' '}
                        {parseFloat(pool.feeAPR || p.feeAPR || '0').toFixed(2)}%
                      </div>
                      <div>
                        Farm APR:{' '}
                        {parseFloat(pool.farmAPR || p.farmAPR || '0').toFixed(
                          2
                        )}
                        %
                      </div>
                      <div className='font-semibold text-green-600'>
                        Daily Volume: $
                        {formatNumber(
                          pool.dailyVolumeUSD || p.dailyVolumeUSD || '0'
                        )}
                      </div>
                      <div>
                        TVL: ${formatNumber(pool.tvlUSD || p.tvlUSD || '0')}
                      </div>
                    </div>
                  </div>
                );
            ...
            <div>
              <h3 className='font-semibold mb-2'>
                Transformed Data for Investment Ideas
              </h3>
              <div className='text-sm text-muted-foreground mb-2'>
          ...
          <div className='mt-6 p-4 bg-gray-50 rounded'>
            <h3 className='font-semibold mb-3'>
              Data Structure for Investment Ideas
            </h3>
            <div className='text-sm space-y-2'>
              <p>
                <strong>Raw Hyperion Pool Structure:</strong>
              </p>
  ...
  tvlUSD: string           // Total Value Locked in USD
              ...
              <p>
                <strong>Transformed InvestmentData Structure:</strong>
              </p>
              <pre className='bg-white p-2 rounded text-xs overflow-x-auto'>
                {`{
          ...
          {filteredPools.length === 0 && hyperionPools.length > 0 && (
            <div className='mt-4 p-4 bg-yellow-50 rounded'>
              ...
              <p className='text-sm text-yellow-700'>
                No pools found with daily volume greater than{' '}
                {formatNumber(volumeThreshold)}. Try lowering the threshold or
                check if the pools have volume data.
              </p>
            </div>
          )}
        </CardContent>
