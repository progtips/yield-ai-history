    ...
    <div className='container mx-auto p-6 space-y-6'>
        ...
        <CardContent>
          <div className='space-y-4'>
            ...
            <Button
              onClick={() => {
                ...
                auroPositions.forEach((pos: any) => {
                  const collateralPrice = getAuroTokenPrice(
                    pos.collateralTokenAddress || ''
                  );
                  const debtPrice = getAuroTokenPrice(
                    pos.debtTokenInfo?.faAddress || ''
                  );
            ...
            <Button
              onClick={() => {
                console.log(
                  'Current Hyperion prices in store:',
                  hyperionPrices
                );
                console.log('Hyperion Positions:', hyperionPositions);
                hyperionPositions.forEach((pos: any) => {
                  const token1Price = getHyperionTokenPrice(
                    pos.position?.pool?.token1 || ''
                  );
                  const token2Price = getHyperionTokenPrice(
                    pos.position?.pool?.token2 || ''
                  );
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                ...
                {auroPositions
                  ...
                  .map((position: any, index: number) => (
                    <div key={index} className='border p-2 rounded'>
                      ...
                      <div className='text-sm'>
                        <strong>Collateral:</strong> {position.collateralAmount}{' '}
                        {position.collateralSymbol}
                      </div>
                      <div className='text-sm'>
                        <strong>Debt:</strong> {position.debtAmount}{' '}
                        {position.debtSymbol}
                      </div>
                    </div>
                  ))}
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Hyperion Positions ({hyperionPositions.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                {hyperionPositions
                  ...
                  .map((position: any, index: number) => (
                    <div key={index} className='border p-2 rounded'>
                      <div className='text-sm'>
                        <strong>Pool:</strong>{' '}
                        {position.position?.pool?.token1Info?.symbol}/
                        {position.position?.pool?.token2Info?.symbol}
                      </div>
                      <div className='text-sm'>
                        <strong>Value:</strong> $
            ...
            {hyperionPools.length === 0 ? (
              ...
              <div className='space-y-2'>
                {hyperionPools.slice(0, 3).map((pool: any, index: number) => (
                  <div key={index} className='border p-2 rounded'>
                    <div className='text-sm'>
                      <strong>Pool:</strong> {pool.token1Info?.symbol}/
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Auro Prices ({Object.keys(auroPrices).length})
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Hyperion Prices ({Object.keys(hyperionPrices).length})
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              Auro Rewards ({Object.keys(rewards).length} positions)
            </CardTitle>
          </CardHeader>
          <CardContent>
            {Object.keys(rewards).length === 0 ? (
              ...
              <div className='space-y-2'>
                {Object.entries(rewards)
                  .slice(0, 3)
                  .map(([positionAddress, positionRewards]) => (
                    <div key={positionAddress} className='border p-2 rounded'>
                      ...
                      <div className='text-sm'>
                        <strong>Collateral Rewards:</strong>{' '}
                        {positionRewards.collateral.length}
                      </div>
                      <div className='text-sm'>
                        <strong>Borrow Rewards:</strong>{' '}
                        {positionRewards.borrow.length}
                      </div>
                    </div>
                  ))}
          ...
          <CardContent>
            <div className='space-y-2'>
              ...
              <div className='text-sm'>
                ...
                {hyperionTotalRewards.toFixed(2)}
              </div>
              <div className='text-sm'>
                <strong>Active Positions:</strong>{' '}
                {hyperionPositions.filter((p: any) => p.isActive).length}
              </div>
            </div>
          </CardContent>
        </Card>
