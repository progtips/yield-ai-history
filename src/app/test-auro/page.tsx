...
export default function TestAuroPage() {
  ...
  const handleFetchPositions = async () => {
    ...
    try {
      const response = await fetch(
        `/api/protocols/auro/userPositions?address=${encodeURIComponent(walletAddress)}`
      );
      const data = await response.json();
  ...
  const filteredPools = poolsData.filter(pool => {
    ...
    if (showOnlyStablePools) {
      const isStable =
        pool.collateralTokenSymbol?.toUpperCase().includes('USDT') ||
  ...
  // Group pools by type
  const collateralPools = filteredPools.filter(
    pool => pool.type === 'COLLATERAL'
  );
  ...
  return (
    <div className='container mx-auto p-6 space-y-6'>
      ...
      <div className='flex items-center gap-4'>
        ...
        <div>
          <h1 className='text-3xl font-bold'>
            Auro Finance - Pro Investment Ideas
          </h1>
      ...
      <Tabs
        defaultValue='pools'
        className='w-full'
        onValueChange={value => setActiveTab(value as 'pools' | 'positions')}
      >
                ...
                <div>
                  ...
                  <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
                    {collateralPools
                      .sort(
                        (a, b) =>
                          (b.totalSupplyApr || 0) - (a.totalSupplyApr || 0)
                      )
                      .map((pool, index) => (
                        <Card
                            ...
                            <CardTitle className='flex items-center justify-between'>
                              <div className='flex items-center gap-2'>
                                <Avatar className='h-6 w-6'>
                                  ...
                                  />
                                  <AvatarFallback>
                                    {pool.collateralTokenSymbol?.slice(0, 2)}
                                  </AvatarFallback>
                                </Avatar>
                            ...
                            <div className='mt-3 space-y-1 text-xs'>
                              ...
                              {pool.supplyIncentiveApr && (
                                <div className='flex justify-between'>
                                  ...
                                  <span>
                                    {pool.supplyIncentiveApr.toFixed(2)}%
                                  </span>
                                </div>
                              )}
                              {pool.stakingApr && (
                ...
                <div>
                  ...
                  <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
                    {borrowPools
                      .sort(
                        (a, b) =>
                          (b.totalBorrowApr || 0) - (a.totalBorrowApr || 0)
                      )
                      .map((pool, index) => (
                        <Card
                          key={pool.poolAddress}
                          ...
                          <CardHeader>
                            <CardTitle className='flex items-center justify-between'>
                              <div className='flex items-center gap-2'>
                                <Avatar className='h-6 w-6'>
                                  <AvatarImage
                                    src={getTokenLogo(pool.token?.address)}
                                  />
                                  <AvatarFallback>
                                    {pool.token?.symbol?.slice(0, 2)}
                                  </AvatarFallback>
                                </Avatar>
                                <span className='text-sm'>
                                  {pool.token?.symbol}
                            ...
                            <div className='mt-3 space-y-1 text-xs'>
                              ...
                              {pool.borrowIncentiveApr && (
                                <div className='flex justify-between'>
                                  <span>Incentive APR:</span>
                                  <span>
                                    {pool.borrowIncentiveApr.toFixed(2)}%
                                  </span>
                                </div>
                              )}
                            </div>
                      ...
                      <CardTitle className='flex items-center justify-between'>
                        <div className='flex items-center gap-2'>
                          <Avatar className='h-6 w-6'>
                            ...
                            />
                            <AvatarFallback>
                              {position.collateralSymbol?.slice(0, 2)}
                            </AvatarFallback>
                          </Avatar>
