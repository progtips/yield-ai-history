...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      'https://api.amnis.finance/api/v1/staking-ami/pools',
      {
        method: 'GET',
        headers: {
    ...
    const transformedPools = data.map((pool: any) => ({
      ...
      endDate:
        pool.endTime > 0
          ? new Date(pool.endTime * 1000).toLocaleDateString()
          : 'No end date',
      ...
      status: pool.isPause
        ? 'Paused'
        : pool.startTime > Math.floor(Date.now() / 1000)
          ? 'Upcoming'
          : pool.endTime > 0 && pool.endTime < Math.floor(Date.now() / 1000)
            ? 'Ended'
            : 'Active',
    }));
