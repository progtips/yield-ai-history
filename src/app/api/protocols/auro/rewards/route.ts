...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !positionsInfo ||
      !Array.isArray(positionsInfo) ||
      positionsInfo.length === 0
    ) {
      return NextResponse.json(
        { success: false, error: 'positionsInfo is required' },
        { status: 400 }
      );
    }
    if (!poolsData || !Array.isArray(poolsData) || poolsData.length === 0) {
      return NextResponse.json(
        { success: false, error: 'poolsData is required' },
        { status: 400 }
      );
    }
    ...
    for (const pos of positionsInfo) {
      ...
      const collateralPool = poolsData.find(
        p => p.poolAddress === pos.poolAddress
      );
      console.log(
        'Найденный collateral pool:',
        collateralPool
          ? {
      ...
      if (collateralPool && collateralPool.rewardPoolAddress) {
        pairs.push({
          ...
          pool: collateralPool.rewardPoolAddress,
        });
        console.log('Добавлена collateral пара:', {
          position: pos.address,
          pool: collateralPool.rewardPoolAddress,
        });
      }
      ...
      if (hasDebt) {
        ...
        if (borrowPool && borrowPool.borrowRewardsPoolAddress) {
          pairs.push({
            position: pos.address,
            pool: borrowPool.borrowRewardsPoolAddress,
          });
          console.log('Добавлена borrow пара:', {
            position: pos.address,
            pool: borrowPool.borrowRewardsPoolAddress,
          });
        } else {
          console.log(
            'Borrow пара НЕ добавлена - нет borrowPool или borrowRewardsPoolAddress'
          );
        }
      } else {
      ...
      try {
        ...
        if (single && Array.isArray(single) && single.length > 0) {
          ...
          if (positionData) {
            ...
            const isBorrowPool = poolsData.find(
              p => p.borrowRewardsPoolAddress === pool
            );
            if (isBorrowPool) {
    ...
    const processedPositionRewards: {
      [positionAddress: string]: { collateral: any[]; borrow: any[] };
    } = {};

    for (const [positionAddress, rewards] of Object.entries(
      positionRewardsMap
    )) {
