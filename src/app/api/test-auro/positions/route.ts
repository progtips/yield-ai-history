...
export async function GET(request: NextRequest) {
  try {
    ...
    const positions = (data.data?.current_token_ownerships_v2 || []).map(
      (x: any) => x.storage_id
    );
...
async function getPositionInfo(positionsAddress: string[]): Promise<any[]> {
  try {
    ...
    const [rawPositionInfo] = await aptos.view({
      payload: payloadPositionInfo,
    });
    ...
    if (rawPositionInfo && Array.isArray(rawPositionInfo)) {
      return rawPositionInfo.map((position: any, index: number) => ({
        ...
        collateralAmount: position.asset_amount
          ? (parseInt(position.asset_amount) / 1e8).toFixed(2)
          : '0',
        debtAmount: position.debt_amount
          ? (parseInt(position.debt_amount) / 1e8).toFixed(2)
          : '0',
        liquidatePrice: position.liquidate_price
          ? (parseInt(position.liquidate_price) / 1e8).toFixed(2)
          : '0',
      }));
    }
        ...
        if (positionData.data?.current_token_ownerships_v2?.[0]) {
          ...
          return {
            ...
            collateralAmount: position.amount
              ? (parseInt(position.amount) / 1e8).toFixed(2)
              : '0',
