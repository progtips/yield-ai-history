...
export default function TestEchelonPage() {
  ...
  const [claimingRewards, setClaimingRewards] = useState<Set<string>>(
    new Set()
  );
  ...
  const [collateralMarketsData, setCollateralMarketsData] =
    useState<string>('');
  const [collateralMarketsLoading, setCollateralMarketsLoading] =
    useState(false);
  ...
  const fetchTokenPrices = async (tokens: string[]) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/panora/prices?chainId=1&addresses=${tokenAddresses.join(',')}`
      );
  ...
  const handleFetchRewards = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const uniqueTokens = [
          ...new Set(rewardsData.map((r: EchelonReward) => r.token)),
        ] as string[];
  ...
  const handleFetchCollateralMarkets = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || 'Failed to fetch collateral markets data'
        );
      }
  ...
  const parseVaultData = (vaultResource: any) => {
    ...
    const marketData = [
      {
        market:
          '0x778362f04f7904ba0b76913ec7c0c5cc04e469b0b96929c6998b34910690a740',
        coin: '0xb30a694a344edee467d9f82330bbe7c3b89f440a1ecd2da1f3bca266560fce69',
      },
      {
        market:
      ...
      {
        market:
          '0x2c4e0bb55272f9c120ffd5a414c10244005caf9c1b14527cea3df7074c5bf623',
        coin: '0xbae207659db88bea0cbead6da0ed00aac12edcdda169e591cd41c94180b46f3b',
      },
      {
        market:
    ...
    if (vaultResource.data.collaterals?.data) {
      result.collaterals = vaultResource.data.collaterals.data.map(
        (item: any) => {
    ...
    if (vaultResource.data.liabilities?.data) {
      result.liabilities = vaultResource.data.liabilities.data.map(
        (item: any) => {
          const marketAddress = item.key.inner;
          const market = marketData.find(m => m.market === marketAddress);
          const coinAddress = market?.coin || 'Unknown';
          ...
          return {
            ...
            interestAccumulated: {
              ...
              amount:
                Number(item.value.interest_accumulated) /
  ...
  const getTokenIcon = (tokenName: string) => {
    if (
      tokenName.toLowerCase().includes('aptos') ||
      tokenName.toLowerCase().includes('apt')
    ) {
      return '🟢';
    }
    if (
      tokenName.toLowerCase().includes('thala') ||
      tokenName.toLowerCase().includes('thapt')
    ) {
      return '🔵';
    }
    return '🪙';
  ...
  const formatFarmingId = (farmingId: string) => {
    if (farmingId.startsWith('@')) {
      return (
        farmingId.substring(1, 10) +
        '...' +
        farmingId.substring(farmingId.length - 8)
      );
    }
    return (
      farmingId.substring(0, 10) +
      '...' +
      farmingId.substring(farmingId.length - 8)
    );
  };
  ...
  const handleClaimReward = async (reward: EchelonReward) => {
    ...
    try {
      ...
      const txResponse = await signAndSubmitTransaction({
        data: {
          function: data.data.transactionPayload
        ...
        for (let i = 0; i < maxAttempts; i++) {
          try {
            const txStatusResponse = await fetch(
              `https://fullnode.mainnet.aptoslabs.com/v1/transactions/by_hash/${txResponse.hash}`
            );
          ...
          <div className='grid gap-4'>
            ...
            <div className='flex gap-2'>
              ...
              <Button
                ...
                variant='outline'
              >
                {collateralMarketsLoading
                  ? 'Loading...'
                  : 'Fetch Collateral Markets'}
              </Button>
          ...
          {!account?.address && (
            <div className='p-4 border rounded-lg bg-yellow-50 border-yellow-200'>
              <p className='text-yellow-800'>
                <strong>Wallet Required:</strong> Connect your wallet to claim
                rewards. You can still view rewards by entering an address
                manually.
              </p>
            </div>
          )}
        ...
        <Card>
          ...
          <CardContent>
            <div className='p-4 border rounded-lg bg-blue-50 border-blue-200'>
              <p className='text-sm font-mono text-blue-700 whitespace-pre-wrap'>
                {vaultData}
              </p>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className='p-4 border rounded-lg bg-green-50 border-green-200'>
              <p className='text-sm font-mono text-green-700 whitespace-pre-wrap'>
                {parsedVaultData}
              </p>
            </div>
          </CardContent>
                  ...
                  <TableRow key={index}>
                        ...
                        {pricesLoading
                          ...
                          : (() => {
                              ...
                              if (
                                price &&
                                price.usdPrice &&
                                !isNaN(parseFloat(price.usdPrice))
                              ) {
                                const usdValue =
                                  reward.amount * parseFloat(price.usdPrice);
                    ...
                    <TableCell className='text-right'>
                      <Button
                        ...
                        disabled={claimingRewards.has(
                          `${reward.farmingId}-${reward.token}`
                        )}
                      >
                        {claimingRewards.has(
                          `${reward.farmingId}-${reward.token}`
