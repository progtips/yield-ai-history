...
export function InvestmentsDashboard({ className }: InvestmentsDashboardProps) {
      ...
      try {
          ...
          },
          {
            ...
            transform: (data: any) => {
              ...
              return collateralPools.map((pool: any) => {
                ...
                const supplyIncentiveApr = parseFloat(
                  pool.supplyIncentiveApr || '0'
                );
        ...
        const fetchPromises = protocolEndpoints.map(async endpoint => {
          try {
            ...
            console.log(
              `${endpoint.name} loaded: ${transformedData.length} pools`
            );
            ...
            console.log(
              `${endpoint.name} completed: ${transformedData.length} pools`
            );
            return {
              name: endpoint.name,
            ...
            console.error(
              `Error fetching ${endpoint.name} from ${endpoint.url}:`,
              error
            );
  ...
  const filteredData = allLoadedData.filter(item => {
    ...
    if (
      item.protocol === 'Echelon' &&
      EXCLUDED_ECHELON_TOKENS.includes(item.token)
    ) {
      return false;
    }
  ...
  const showLoadingIndicators =
    loading && Object.values(protocolsLoading).some(Boolean);
            ...
            <div className='space-y-6'>
              ...
              <div className='mt-6'>
                ...
                <div className='space-y-2'>
                  {Object.entries(protocolsLoading).map(
                    ([protocolName, isLoading]) => (
                      <div
                        key={protocolName}
                        ...
                        {isLoading ? (
                          ...
                          <>
                            ...
                            <span className='text-red-500'>
                              {protocolName}: {protocolsError[protocolName]}
                            </span>
                          </>
                        ) : (
                          <>
                            ...
                            <span className='text-green-600'>
                              {protocolName}:{' '}
                              {protocolsData[protocolName]?.length || 0} pools
                              loaded
                            </span>
                          </>
                        )}
              ...
              <div className='flex items-center gap-1'>
                {Object.values(protocolsLoading).filter(Boolean).length > 0 && (
                  <>
                    ...
                    <span>
                      Loading{' '}
                      {Object.values(protocolsLoading).filter(Boolean).length}{' '}
                      protocols...
                    </span>
                  </>
                )}
              ...
              <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
                {allLoadedData
                  .filter(item => {
                    ...
                    return (
                      item.asset.toUpperCase().includes('USDT') ||
                      ...
                      >
                                  ...
                                  <div className='flex items-center gap-2'>
                                    {isDex ? (
                                      ...
                                      <div className='flex items-center gap-2'>
                                        ...
                                        <span>
                                          {item.token1Info?.symbol || '-'}
                                        </span>
                                        <span className='text-gray-400'>/</span>
                                        <span>
                                          {item.token2Info?.symbol || '-'}
                                        </span>
                                      </div>
                                    ) : (
                                  ...
                                  <div className='space-y-1'>
                                    ...
                                    {isDex ? (
                                      ...
                                      <>
                                        ...
                                        <p className='text-xs'>
                                          Token 1: {item.token1Info?.symbol} (
                                          {item.token1Info?.name})
                                        </p>
                                        <p className='text-xs'>
                                          Token 2: {item.token2Info?.symbol} (
                                          {item.token2Info?.name})
                                        </p>
                                        {item.dailyVolumeUSD && (
                                          <p className='text-xs'>
                                            Volume: $
                                            {item.dailyVolumeUSD.toLocaleString()}
                                          </p>
                                        )}
                                        {item.tvlUSD && (
                                          <p className='text-xs'>
                                            TVL: ${item.tvlUSD.toLocaleString()}
                                          </p>
                                        )}
                                      </>
                                    ) : (
                        ...
                        <CardContent>
                          ...
                          <DepositButton
                            ...
                            tokenIn={{
                              symbol: isDex
                                ? item.token1Info?.symbol || 'Unknown'
                                : displaySymbol,
                              logo: isDex
                                ? item.token1Info?.logoUrl || ''
                                : tokenInfo?.logoUrl || '',
                              decimals: isDex
                                ? item.token1Info?.decimals || 8
                ...
                {[
                  ...
                  return (
                    ...
                    >
                                ...
                                <div className='flex items-center gap-2'>
                                  {isDex ? (
                                    ...
                                    <div className='flex items-center gap-2'>
                                      ...
                                      <span>
                                        {bestPool.token1Info?.symbol || '-'}
                                      </span>
                                      <span className='text-gray-400'>/</span>
                                      <span>
                                        {bestPool.token2Info?.symbol || '-'}
                                      </span>
                                    </div>
                                  ) : (
                                    // Lending pool display (existing logic)
                                ...
                                <div className='space-y-1'>
                                  ...
                                  {isDex ? (
                                    ...
                                    <>
                                      ...
                                      <p className='text-xs'>
                                        Token 1: {bestPool.token1Info?.symbol} (
                                        {bestPool.token1Info?.name})
                                      </p>
                                      <p className='text-xs'>
                                        Token 2: {bestPool.token2Info?.symbol} (
                                        {bestPool.token2Info?.name})
                                      </p>
                                      {bestPool.dailyVolumeUSD && (
                                        <p className='text-xs'>
                                          Volume: $
                                          {bestPool.dailyVolumeUSD.toLocaleString()}
                                        </p>
                                      )}
                                      {bestPool.tvlUSD && (
                                        <p className='text-xs'>
                      ...
                      <CardContent>
                        <div className='text-2xl font-bold'>
                          {bestPool.totalAPY?.toFixed(2) || '0.00'}%
                        </div>
                        <p className='text-xs text-muted-foreground'>
                          Total APY
                        </p>
                        <DepositButton
                          ...
                          tokenIn={{
                            symbol: isDex
                              ? bestPool.token1Info?.symbol || 'Unknown'
                              : displaySymbol,
                            logo: isDex
                              ? bestPool.token1Info?.logoUrl || ''
                              : tokenInfo?.logoUrl || '',
            ...
            <div className='flex items-center space-x-2'>
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setShowOnlyStablePools(checked as boolean)
                }
              />
              <label
                ...
                <TableRow>
                  ...
                  <TableHead>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        APR - Annual % cost or reward from borrowing
                      </TooltipContent>
                    </Tooltip>
                  </TableHead>
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    if (item.protocol === 'Hyperion') {
                                      window.open(
                                        `https://hyperion.xyz/pool/${item.token}`,
                                        '_blank'
                                      );
                                    } else if (
                                      item.protocol === 'Tapp Exchange'
                                    ) {
                                      window.open(
                                        `https://tapp.exchange/pool`,
                                        '_blank'
                                      );
                                    }
                                  }}
