...
export function AnalyticsKPICards({ data }: AnalyticsKPICardsProps) {
  ...
  const tpsChange =
    data.tps.length >= 2
      ? ((data.tps[data.tps.length - 1].tps - data.tps[0].tps) /
          data.tps[0].tps) *
        100
      : 0;
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
      ...
      <Card>
        ...
        <CardContent>
          <div className='text-2xl font-bold'>
            {formatPercentage(data.successRate.successRate)}
          </div>
          <div className='flex items-center gap-2 text-xs text-muted-foreground'>
            <span>
              {formatNumber(data.successRate.successfulTransactions)} successful
            </span>
            <span>•</span>
            <span>
              {formatNumber(data.successRate.failedTransactions)} failed
            </span>
          </div>
          <p className='text-xs text-muted-foreground mt-1'>
            {formatNumber(data.successRate.totalTransactions)} total
            transactions
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className='text-2xl font-bold'>
            {formatGas(data.gasStats.averageGasUsed)}
          </div>
          <div className='flex items-center gap-2 text-xs text-muted-foreground'>
          ...
          {data.topModules.length > 0 ? (
            <>
              <div
                ...
                title={data.topModules[0].module}
              >
                {data.topModules[0].module.split('::')[1] ||
                  data.topModules[0].module}
              </div>
              <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                <span>
                  {formatNumber(data.topModules[0].transactionCount)}{' '}
                  transactions
                </span>
                <span>•</span>
                <span>
                  {formatPercentage(data.topModules[0].percentage)} of total
                </span>
              </div>
              <p className='text-xs text-muted-foreground mt-1'>
