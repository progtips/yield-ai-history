...
export function PositionsList({
    ...
    async function loadPositions() {
      ...
      try {
        ...
        const response = await fetch(
          `/api/protocols/tapp/userPositions?address=${walletAddress}`
        );
  ...
  const sortedPositions = [...positions].sort((a, b) => {
    const aValue =
      a.estimatedWithdrawals.reduce(
        (sum, token) => sum + parseFloat(token.usd || '0'),
        0
      ) +
      a.estimatedIncentives.reduce(
        (sum, incentive) => sum + parseFloat(incentive.usd || '0'),
        0
      );
    const bValue =
      b.estimatedWithdrawals.reduce(
        (sum, token) => sum + parseFloat(token.usd || '0'),
        0
      ) +
      b.estimatedIncentives.reduce(
        (sum, incentive) => sum + parseFloat(incentive.usd || '0'),
        0
      );
