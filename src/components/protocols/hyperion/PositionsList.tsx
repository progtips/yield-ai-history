...
export function PositionsList({
    ...
    async function loadPositions() {
      ...
      try {
        ...
        const response = await fetch(
          `/api/protocols/hyperion/userPositions?address=${walletAddress}`
        );
  ...
  const sortedPositions = [...positions].sort((a, b) => {
    const aValue =
      parseFloat(a.value || '0') + parseFloat(a.rewards?.value || '0');
    const bValue =
      parseFloat(b.value || '0') + parseFloat(b.rewards?.value || '0');
