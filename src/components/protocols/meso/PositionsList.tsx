...
function getTokenInfo(tokenAddress: string) {
  const token = (
    ...
    t => t.tokenAddress === tokenAddress || t.faAddress === tokenAddress
  );
...
export function PositionsList({
    ...
    positions.forEach(position => {
      ...
      if (mesoToken?.tokenAddress) {
        const amount = parseFloat(
          formatTokenAmount(position.balance, position.assetInfo.decimals)
        );
  ...
  const sortedPositions = [...positions].sort((a, b) => {
    ...
    const priceA = mesoTokenA?.tokenAddress
      ? getTokenPrice(mesoTokenA.tokenAddress)
      : '0';
    const priceB = mesoTokenB?.tokenAddress
      ? getTokenPrice(mesoTokenB.tokenAddress)
      : '0';
