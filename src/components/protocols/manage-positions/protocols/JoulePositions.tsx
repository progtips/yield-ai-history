...
export function JoulePositions() {
    ...
    async function loadPositions() {
      ...
      try {
        ...
        const response = await fetch(
          `/api/protocols/joule/userPositions?address=${account.address}`
        );
        if (!response.ok) {
        ...
        if (data.userPositions?.[0]?.positions_map?.data) {
          const positionsData = data.userPositions[0].positions_map.data.map(
            (item: any) => item.value
          );
  ...
  useEffect(() => {
    fetch('/api/aptos/pools')
      ...
      .then(data =>
        setMarketData(
          data.data?.filter((m: any) => m.protocol === 'Joule') || []
        )
      );
  }, []);

  // Получить APY для supply/borrow
  const getApyForToken = (
    tokenAddress: string,
    type: 'supply' | 'borrow',
    faAddress?: string
  ) => {
    const market = marketData.find(
      (m: any) =>
        m.token === tokenAddress || (faAddress && m.token === faAddress)
    );
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <div>
                            ...
                            <div className='text-base text-muted-foreground mt-0.5'>
                              $
                              {tokenInfo?.usdPrice
                                ? parseFloat(tokenInfo.usdPrice).toFixed(2)
                                : 'N/A'}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          <div>
                            <div className='flex items-center gap-2'>
                              <span className='text-lg'>
                                {tokenInfo?.symbol ||
                                  borrow.value.coin_name.split('::').pop()}
                              </span>
                              <Badge
                                variant='outline'
                                className='bg-red-500/10 text-red-500 border-red-500/20 text-xs font-normal px-2 py-0.5 h-5'
                              >
                                Borrow
                              </Badge>
                            </div>
