...
const HyperionPosition = memo(function HyperionPosition({
  position,
  index,
}: HyperionPositionProps) {
  ...
  const componentId = useMemo(
    () => `hyperion-${position.position?.objectId}-${index}`,
    [position.position?.objectId, index]
  );
  ...
  const handleViewPoolDetails = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/protocols/hyperion/pools/${position.position.pool.poolId}`
      );
  ...
  const handleRemoveLiquidity = async () => {
    console.log(
      'HyperionPosition: handleRemoveLiquidity called for component',
      componentId
    );
      ...
      <div className='hidden md:flex justify-between items-start'>
        <div className='flex-1'>
          <div className='flex items-center gap-2 mb-2'>
            ...
            {position.position?.pool?.token1Info?.logoUrl &&
              position.position?.pool?.token2Info?.logoUrl && (
            ...
            <span className='text-lg font-semibold'>
              {position.position?.pool?.token1Info?.symbol || 'Unknown'} /{' '}
              {position.position?.pool?.token2Info?.symbol || 'Unknown'}
            </span>
            <TooltipProvider>
              {position.isActive ? (
                <Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Badge
                      variant='outline'
                      className='bg-red-500/10 text-red-600 border-red-500/20 text-xs font-normal px-2 py-0.5 h-5 ml-2 cursor-help'
                    >
                      Inactive
                    </Badge>
                  </TooltipTrigger>
        ...
        <div className='flex flex-col items-end gap-2'>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <div className='space-y-1'>
                    ...
                    <p className='text-xs'>
                      Fee APR: {poolAPR.feeAPR.toFixed(2)}%
                    </p>
                    <p className='text-xs'>
                      Farm APR: {poolAPR.farmAPR.toFixed(2)}%
                    </p>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <div className='space-y-1'>
                    ...
                    {position.farm?.unclaimed?.length > 0 && (
                      <p className='text-xs'>
                        💰 Farm rewards: ${farmRewards.toFixed(2)}
                      </p>
                    )}
                    {position.fees?.unclaimed?.length > 0 && (
                      <p className='text-xs'>
                        💸 Fee rewards: ${feeRewards.toFixed(2)}
                      </p>
                    )}
                    <p className='text-xs font-semibold'>
          ...
          <div className='flex gap-2'>
            ...
            <button
              ...
              disabled={
                isRemoving ||
                !getTokenAddress(position.position.pool.token1Info) ||
                !getTokenAddress(position.position.pool.token2Info)
              }
            >
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            <span className='text-xs text-gray-500'>
              Fee: {poolAPR.feeAPR.toFixed(2)}% | Farm:{' '}
          ...
          <div className='bg-white rounded-lg p-6 max-w-2xl max-h-[80vh] overflow-y-auto'>
            ...
            <div className='space-y-4'>
              {poolDetails.map((pool: any, index: number) => {
                const { feeAPR, farmAPR, tvlUSD, dailyVolumeUSD, feesUSD } =
                  pool;
                return (
                  <div key={index} className='border rounded p-4'>
                    <div className='grid grid-cols-2 gap-4 mb-4'>
                      ...
                      <div>
                        ...
                        <p className='text-sm'>
                          $
                          {parseFloat(tvlUSD || '0').toLocaleString(undefined, {
                      ...
                      <div>
                        ...
                        <p className='text-sm'>
                          $
                          {parseFloat(dailyVolumeUSD || '0').toLocaleString(
                            undefined,
                            { maximumFractionDigits: 2 }
                          )}
                        </p>
                      ...
                      <div>
                        ...
                        <p className='text-sm'>
                          $
                          {parseFloat(feesUSD || '0').toLocaleString(
                            undefined,
                            { maximumFractionDigits: 2 }
                          )}
                        </p>
                    ...
                    <div className='grid grid-cols-2 gap-4'>
                      ...
                      <div>
                        ...
                        <div className='bg-gray-50 p-3 rounded flex items-center gap-2'>
                          ...
                          <div>
                            <div className='font-bold'>
                              {pool.token2Info?.symbol}
                            </div>
                            <div className='text-xs text-gray-500'>
                              {pool.token2Info?.name}
                            </div>
                            <div className='text-xs font-mono break-all'>
                              {pool.token2}
                            </div>
...
export function HyperionPositions() {
  ...
  const loadPositions = async () => {
    ...
    try {
      ...
      setError(null);
      console.log(
        '[HyperionPositions] Fetching:',
        `/api/protocols/hyperion/userPositions?address=${account.address}`
      );
      const response = await fetch(
        `/api/protocols/hyperion/userPositions?address=${account.address}`
      );
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        const duplicates = positionIds.filter(
          (id: string, index: number) => positionIds.indexOf(id) !== index
        );
        ...
        console.log('HyperionPositions: After deduplication', {
          ...
          uniquePositionIds: uniquePositions.map(
            (p: any) => p.position?.objectId
          ),
        });
        ...
        console.log(
          '[HyperionPositions] No valid data, setting positions to []'
        );
        setPositions([]);
      }
    } catch (err) {
