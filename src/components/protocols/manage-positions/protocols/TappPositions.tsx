...
function TappPosition({ position, index }: TappPositionProps) {
  ...
  const rewardsValue = (position.estimatedIncentives || []).reduce(
    (sum: number, r: any) => sum + parseFloat(r.usd || '0'),
    0
  );
  ...
  return (
    <div
      key={`${position.positionAddr}-${index}`}
    ...
    >
      <div className='flex justify-between items-center mb-3'>
        <div className='flex items-center gap-2'>
          ...
          <span className='px-2 py-1 rounded bg-green-500/10 text-green-600 text-xs font-semibold ml-2'>
            Active
          </span>
        </div>
        <div className='flex items-center gap-2'>
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <div className='space-y-1'>
                  ...
                  <p className='text-xs'>
                    Boosted APR: {boostedApr.toFixed(2)}%
                  </p>
              ...
              {rewards.map((reward: any, rewardIndex: number) => (
                ...
                >
                  ...
                  <span className='font-semibold'>
                    {reward.amount.toFixed(6)} {reward.symbol}
                  </span>
                </div>
              ))}
            </div>
...
export function TappPositions() {
  ...
  const loadPositions = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/protocols/tapp/userPositions?address=${account.address}`
      );
      ...
      if (rawData.success && Array.isArray(rawData.data)) {
        const sortedPositions = [...rawData.data].sort((a, b) => {
          const aValue = (a.estimatedWithdrawals || []).reduce(
            (sum: number, token: any) => sum + parseFloat(token.usd || '0'),
            0
          );
          const bValue = (b.estimatedWithdrawals || []).reduce(
            (sum: number, token: any) => sum + parseFloat(token.usd || '0'),
            0
          );
  ...
  const totalValue = positions.reduce((sum, position) => {
    const positionValue = (position.estimatedWithdrawals || []).reduce(
      (tokenSum: number, token: any) => {
        return tokenSum + parseFloat(token.usd || '0');
      },
      0
    );
    const incentivesValue = (position.estimatedIncentives || []).reduce(
      (incentiveSum: number, incentive: any) => {
