...
export function AuroPositions({
  ...
  const [rewardsData, setRewardsData] = useState<{
    [positionAddress: string]: { collateral: any[]; borrow: any[] };
  }>({});
  ...
  const getRewardTokenInfoHelper = useCallback(
    (tokenAddress: string) => {
      console.log(
        '[Auro Managing] getRewardTokenInfoHelper called for:',
        tokenAddress
      );
      const cleanAddress = tokenAddress.startsWith('@')
        ? tokenAddress.slice(1)
        : tokenAddress;
      const fullAddress = cleanAddress.startsWith('0x')
        ? cleanAddress
        : `0x${cleanAddress}`;
      console.log(
        '[Auro Managing] Looking for token with address:',
        fullAddress
      );
      ...
      if (!token) {
        console.log(
          '[Auro Managing] Token not found for address:',
          fullAddress
        );
        return undefined;
      }
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      console.log(
        '[Auro Managing] Token addresses for Panora:',
        addressesArray
      );
          ...
          if (reward && reward.key && reward.value) {
            ...
            if (tokenInfo && tokenInfo.price) {
              const amount =
                ...
                Math.pow(10, tokenInfo.decimals || 8);
  ...
  const sortedPositions = [...positions].sort((a, b) => {
    const collateralPriceA = a.collateralTokenAddress
      ? parseFloat(getTokenPrice(a.collateralTokenAddress))
      : 0;
    const collateralPriceB = b.collateralTokenAddress
      ? parseFloat(getTokenPrice(b.collateralTokenAddress))
      : 0;
  ...
  useEffect(() => {
    const total = sortedPositions.reduce((sum, pos) => {
      const collateralPrice = pos.collateralTokenAddress
        ? parseFloat(getTokenPrice(pos.collateralTokenAddress))
        : 0;
      const collateralValue =
        parseFloat(pos.collateralAmount) * collateralPrice;

      const debtPrice = parseFloat(
        getTokenPrice(
          '0x534e4c3dc0f038dab1a8259e89301c4da58779a5d482fb354a41c08147e6b9ec'
        )
      ); // USDA
  ...
  const getDebtAPRData = () => {
    ...
    if (!borrowPool)
      return { totalApr: 0, borrowApr: 0, borrowIncentiveApr: 0 };
  ...
  const getClaimablePositionsAndTokens = () => {
    ...
    Object.entries(rewardsData).forEach(([positionId, rewards]) => {
      ...
      if (hasRewards) {
        ...
        [...(rewards.collateral || []), ...(rewards.borrow || [])].forEach(
          (reward: any) => {
  ...
  const totalClaimableRewards = Object.keys(rewardsData).reduce(
    (sum, positionId) => {
    ...
    <div className='space-y-4 text-base'>
      <ScrollArea>
        {sortedPositions.map((pos, idx) => {
          ...
          const collateralPrice = pos.collateralTokenAddress
            ? parseFloat(getTokenPrice(pos.collateralTokenAddress)).toFixed(2)
            : 'N/A';
          const collateralValue = pos.collateralTokenAddress
            ? (
                        ...
                        <div className='space-y-3'>
                          ...
                          <div className='text-xs space-y-2 text-gray-200'>
                            <div>
                              ...
                              <code className='bg-gray-800 px-2 py-1 rounded text-xs text-gray-100 block mt-1'>
                                {pos.address}
                              </code>
                            </div>
                            <div>
                          ...
                          <div className='text-xs space-y-1 text-gray-200'>
                            <div>
                              ...
                              {collateralAPRData.supplyApr.toFixed(2)}%
                            </div>
                            <div>
                              <span className='font-medium text-white'>
                                Incentive APR:
                              </span>{' '}
                              {collateralAPRData.supplyIncentiveApr.toFixed(2)}%
                            </div>
                            <div>
                              <span className='font-medium text-white'>
                                Staking APR:
                              </span>{' '}
                              {collateralAPRData.stakingApr.toFixed(2)}%
                            </div>
                            <div className='border-t border-gray-600 pt-1 mt-1'>
                              <span className='font-semibold text-white'>
                                Total APR:{' '}
                                {collateralAPRData.totalApr.toFixed(2)}%
                              </span>
                            </div>
                            {collateralAPRData.rewardPoolAddress && (
                              <div className='mt-2'>
                                ...
                                <code className='bg-gray-800 px-1 rounded text-xs text-gray-100 block mt-1'>
                                  {collateralAPRData.rewardPoolAddress}
                                </code>
                              </div>
                            )}
                          ...
                          <div className='space-y-2'>
                            ...
                            <div className='text-xs space-y-1 text-gray-200'>
                              ...
                              <div>
                                ...
                                {collateralAPRData.supplyIncentiveApr.toFixed(
                                  2
                                )}
                                %
                              </div>
                        ...
                        {rewardsData[pos.address].collateral.length > 0 && (
                          <div className='mb-2'>
                            ...
                            <div className='space-y-1'>
                              {rewardsData[pos.address].collateral.map(
                                (reward, rewardIdx) => {
                                  if (!reward || !reward.key || !reward.value)
                                    return null;
                                  const tokenInfo = getRewardTokenInfoHelper(
                                    reward.key
                                  );
                                  ...
                                  const amount =
                                    parseFloat(reward.value) /
                                    Math.pow(10, tokenInfo.decimals || 8);
                                  const rewardValue = tokenInfo.price
                                    ? (
                                        amount * parseFloat(tokenInfo.price)
                                      ).toFixed(2)
                                    : 'N/A';
                                  return (
                        ...
                        {!hasDebt &&
                          rewardsData[pos.address].borrow.length > 0 && (
                            <div>
                              ...
                              <div className='space-y-1'>
                                {rewardsData[pos.address].borrow.map(
                                  (reward, rewardIdx) => {
                                    if (!reward || !reward.key || !reward.value)
                                      return null;
                                    const tokenInfo = getRewardTokenInfoHelper(
                                    ...
                                    const amount =
                                      parseFloat(reward.value) /
                                      Math.pow(10, tokenInfo.decimals || 8);
                                    const borrowRewardValue = tokenInfo.price
                                      ? (
                                          amount * parseFloat(tokenInfo.price)
                                        ).toFixed(2)
                                      : 'N/A';
                ...
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-2'>
                    ...
                    <span className='text-xs text-gray-500'>
                      Supply: {collateralAPRData.supplyApr.toFixed(2)}% |
                      Incentive:{' '}
                      {collateralAPRData.supplyIncentiveApr.toFixed(2)}%
                    </span>
                  </div>
                        ...
                        {rewardsData[pos.address].collateral.map(
                          (reward, rewardIdx) => {
                            ...
                            const amount =
                              ...
                              Math.pow(10, tokenInfo.decimals || 8);
                            const rewardValue = tokenInfo.price
                              ? (amount * parseFloat(tokenInfo.price)).toFixed(
                                  2
                                )
                              : 'N/A';
                            return (
                              ...
                              >
                                <span>
                                  {amount.toFixed(4)} {tokenInfo.symbol}
                                </span>
                            ...
                            <div className='space-y-2'>
                              ...
                              <div className='text-xs space-y-1 text-gray-200'>
                                <div>
                                  ...
                                  -{debtAPRData.borrowApr.toFixed(2)}%
                                </div>
                                <div>
                                  <span className='font-medium text-white'>
                                    Incentive APR:
                                  </span>{' '}
                                  +{debtAPRData.borrowIncentiveApr.toFixed(2)}%
                                </div>
                                <div className='border-t border-gray-600 pt-1 mt-1'>
                                  <span className='font-semibold text-white'>
                                    Net APR: {debtAPRData.totalApr.toFixed(2)}%
                                  </span>
                                </div>
                                {debtAPRData.rewardPoolAddress && (
                                  <div className='mt-2'>
                ...
                <div className='md:hidden space-y-3 mt-4'>
                  ...
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-2'>
                      ...
                      <span className='text-xs text-gray-500'>
                        Borrow: -{debtAPRData.borrowApr.toFixed(2)}% |
                        Incentive: +{debtAPRData.borrowIncentiveApr.toFixed(2)}%
                      </span>
                    </div>
                    <div className='text-xs text-red-600 font-medium'>
                      Liquidation: ${pos.liquidatePrice}
