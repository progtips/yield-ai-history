          ...
          {overallStats.mostProfitableProtocol && (
            <div className="p-3 bg-green-100 rounded-lg border border-green-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-green-800">Самый прибыльный</span>
              </div>
          ...
          {overallStats.leastProfitableProtocol && (
            <div className="p-3 bg-red-100 rounded-lg border border-red-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-red-800">Наименее прибыльный</span>
              </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {protocolBreakdown.map((protocol) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full ${protocol.isPositive ? 'bg-green-500' : 'bg-red-500'}`} />
                  <span className="font-medium text-sm">{protocol.protocolName}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-medium ${protocol.isPositive ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="text-sm text-blue-700 space-y-1">
            {profitableProtocols.length > unprofitableProtocols.length ? (
              <div>✅ Большинство протоколов приносят прибыль - отличная стратегия!</div>
            ) : (
              <div>⚠️ Большинство протоколов убыточны - рассмотрите ребалансировку портфеля</div>
            )}
